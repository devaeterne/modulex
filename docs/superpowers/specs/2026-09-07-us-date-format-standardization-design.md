...
Modulex will separate **presentation format** from **canonical data format**:
...
- User-facing date-time values use `MM.DD.YYYY` for the date portion and retain an explicit time portion where the product already shows time. The change must not silently remove meaningful time information.
- No database migration is required solely for display formatting.

This design deliberately avoids relying on native `<input type="date">` presentation because browsers/operating systems control that visual format and cannot guarantee the required dotted `MM.DD.YYYY` contract.
...
Rejected. Converting individual pages with ad-hoc `Intl.DateTimeFormat`, `toLocaleDateString`, `new Date("YYYY-MM-DD")`, or string replacement would create inconsistent timezone and parsing behavior and would drift over time.
...
- Preserve the existing product timezone behavior unless a surface already has an explicit business/user timezone setting.
...
All editable date-only fields that currently rely on native date formatting will move to a reviewed shared date input contract.
...
- not create a route-specific visual system;
- preserve existing business validation and mutation ownership.

A calendar popover may be retained or added only through the shared primitive. The textual value shown to the user remains `MM.DD.YYYY` regardless of browser locale.

## Shared display behavior

Feature routes must not hand-roll date display. Shared formatters will cover at least these semantic cases:
...
Names may follow the existing repository naming convention discovered during implementation, but the semantic separation above is mandatory.

## Scope of repository audit

Implementation must audit both applications for user-facing date handling, including:
...
Machine-readable payloads, DB migrations, SQL literals, API contracts, logs intended for machines, filenames, migration timestamps, and test fixtures are not changed merely for visual consistency.

## Admin UI integration

`modulex-admin` must follow `ADMIN_UI_GUIDE.md` and `ADMIN_VALIDATION_GUIDE.md`:
...
`modulex-store` will use the same presentation/data semantics for public, Customer Portal and Dealer Portal surfaces.

Public content dates, if rendered, follow `MM.DD.YYYY`. Portal business dates and timestamped fulfillment/history data follow the same semantic split between calendar dates and moments in time. Access-control and projection boundaries are unchanged.

## Compatibility and data safety

This package is presentation/normalization work, not a data migration.
...
Testing is contract-first and must cover both utility behavior and affected UI surfaces.

Required formatter/parser cases:
...
Repository-level regression must guard against newly introduced locale-dependent date rendering and newly introduced native date inputs outside the reviewed shared primitive.

Final verification includes the affected Admin/Store targeted contracts, Admin strict UI checks, TypeScript, lint and production builds for both applications. Browser/live acceptance should sample representative Admin, Customer Portal and Dealer Portal date-entry/display paths after deployment.
...
4. Run a repository audit for remaining locale-dependent or native date handling and classify intentional machine/internal exceptions.
...
- keep both `[~]` until their required regression and deployed acceptance criteria are complete;
- mark `[x]` only after post-deploy verification.

## Non-goals

- No localization framework is introduced.
- No user-selectable date-format setting is introduced in this package.
- No timezone-setting product feature is introduced.
- No database date/timestamp type conversion is performed solely for presentation.
- No unrelated UI restyling or business-rule refactoring is included.
