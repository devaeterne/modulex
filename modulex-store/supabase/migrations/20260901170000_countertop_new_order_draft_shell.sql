...
-- New Order can bootstrap an empty Draft shell so the canonical Countertop
-- configurator has a saved order/price-group boundary before it creates the
-- authoritative countertop order item. Empty orders remain forbidden for any
-- requested or effective non-Draft status.
...
begin
  ...
  -- Validate the caller-requested state before Sales confirmation can be
  -- converted to Draft for approval. This prevents an empty requested
  -- Confirmed order from entering through the Draft-shell exception.
  if jsonb_array_length(p_items) = 0 and p_initial_status <> 'draft' then
    raise exception 'Empty customer orders are only allowed as Draft countertop shells.';
  end if;
  ...
  if v_role='sales' then
    ...
    if coalesce((v_risk->>'requires_approval')::boolean,false) then
      v_request_id := private.create_approval_request(
        ...
        jsonb_build_object('requested_initial_status',p_initial_status),
        v_risk,
        v_risk->>'approval_key'
      );
