-- Countertop Faucet selection and pricing.
-- Backward compatible: existing public attach/create RPC signatures remain unchanged.
-- Modulex Faucets are priced from active USD Price Group prices.
-- Customer-provided Faucets are project/order history only and never contribute price/reservation.

create or replace function public.calculate_countertop_price_with_faucet(
  ...
  p_faucet_product_id uuid default null
)
returns jsonb
language plpgsql
...
declare
  v_snapshot jsonb;
  v_faucet numeric(18,4) := 0;
  v_faucet_price_source text := null;
begin
  ...
  if p_faucet_product_id is not null then
    perform 1
    from public.products p
    where p.id = p_faucet_product_id
      and p.status = 'active'
      and lower(coalesce(p.metadata->>'product_kind','')) = 'faucet';

    if not found then
      raise exception 'Faucet is unavailable.';
    end if;

    select pp.amount
      into v_faucet
    from public.product_prices pp
    where pp.product_id = p_faucet_product_id
    ...
    if v_faucet is null then
      raise exception 'Faucet has no active price for this price group.';
    end if;
    v_faucet_price_source := 'price_group';
  end if;

  return v_snapshot || jsonb_build_object(
    'faucet_subtotal', v_faucet,
    'faucet_price_source', v_faucet_price_source,
    'subtotal', round((v_snapshot->>'subtotal')::numeric + v_faucet, 4)
  );
end;
$$;

revoke all on function public.calculate_countertop_price_with_faucet(uuid,uuid,uuid,numeric,uuid,numeric,uuid,jsonb,numeric,numeric,uuid) from public;
revoke all on function public.calculate_countertop_price_with_faucet(uuid,uuid,uuid,numeric,uuid,numeric,uuid,jsonb,numeric,numeric,uuid) from anon;
grant execute on function public.calculate_countertop_price_with_faucet(uuid,uuid,uuid,numeric,uuid,numeric,uuid,jsonb,numeric,numeric,uuid) to authenticated;
...
declare
  ...
  v_faucet_product_id uuid;
  v_faucet_source text;
...
begin
  ...
  v_faucet_source := nullif(btrim(coalesce(p_configuration->>'faucet_source','')), '');
  if v_faucet_source is not null and v_faucet_source not in ('modulex', 'customer_provided') then
    raise exception 'Faucet source is invalid.';
  end if;

  if v_faucet_source = 'modulex' then
    begin
      v_faucet_product_id := nullif(p_configuration->>'faucet_product_id','')::uuid;
    exception
      when invalid_text_representation then
        raise exception 'Faucet catalog reference is invalid.';
    end;
    if v_faucet_product_id is null then
      raise exception 'Modulex Faucet requires a catalog product.';
    end if;
  elsif nullif(p_configuration->>'faucet_product_id','') is not null then
    raise exception 'Only Modulex Faucet selection may use faucet_product_id.';
  end if;
  ...
  v_snapshot := public.calculate_countertop_price_with_faucet(
    p_stone_product_id,
    p_material_price_band_id,
    p_price_group_id,
    p_sqft,
  ...
  insert into public.countertop_configurations(
    order_id, order_item_id, stone_product_id, material_price_band_id, sink_product_id,
    price_group_id, edge_profile_id, sqft, edge_linear_ft, slab_quantity,
    manual_price_per_sqft, manual_sink_price, override_reason, overridden_by,
    overridden_at, configuration, pricing_snapshot, subtotal
  ) values (
    v_order_id, p_order_item_id, p_stone_product_id, p_material_price_band_id, p_sink_product_id,
    p_price_group_id, p_edge_profile_id, p_sqft, p_edge_linear_ft, p_slab_quantity,
    p_manual_material_price, v_manual_sink_price, nullif(btrim(p_override_reason), ''),
    ...
    v_configuration, v_snapshot, v_subtotal
  )
...
declare
  ...
  v_faucet_product_id uuid;
  v_customer_faucet_product_id uuid;
  v_customer_faucet_product_name text;
  v_customer_faucet_product_sku text;
  v_customer_faucet_note text;
  v_customer_faucet_display_name text;
begin
  ...
  if new.edge_profile_id is not null then
    select jsonb_build_object(
      'edge_profile_id', ep.id, 'name', ep.name, 'pricing_method', ep.pricing_method,
      'unit_price', ep.unit_price, 'linear_ft', new.edge_linear_ft,
  ...
  if new.configuration->>'sink_source' = 'customer_provided' then
    ...
    if v_customer_sink_product_id is not null then
      select p.name, p.sku into v_customer_sink_product_name, v_customer_sink_product_sku
      from public.products p
      where p.id = v_customer_sink_product_id
        and lower(coalesce(p.metadata->>'product_kind','')) = 'sink';
      if not found then raise exception 'Customer-provided Sink catalog reference is unavailable.'; end if;
    end if;
    ...
    v_customer_sink_display_name := concat_ws(' · ', 'Customer Provides', v_customer_sink_product_name, v_customer_sink_note);
    v_sink_price_source := 'customer_provided';
    v_sink := jsonb_build_object(
      'product_id', null, 'sku', v_customer_sink_product_sku,
      ...
      'price_source', 'customer_provided', 'unit_price', 0, 'subtotal', 0
    );
  else
    ...
    if new.sink_product_id is not null then
      ...
      from public.products p where p.id = new.sink_product_id;
    end if;
  end if;

  if new.configuration->>'faucet_source' = 'customer_provided' then
    if nullif(new.configuration->>'faucet_product_id','') is not null then
      raise exception 'Customer-provided Faucet cannot use Modulex Faucet pricing.';
    end if;
    begin
      v_customer_faucet_product_id := nullif(new.configuration->>'customer_provided_faucet_product_id','')::uuid;
    exception when invalid_text_representation then
      raise exception 'Customer-provided Faucet catalog reference is invalid.';
    end;
    v_customer_faucet_note := nullif(btrim(coalesce(new.configuration->>'customer_provided_faucet_note','')), '');
    if v_customer_faucet_product_id is not null then
      select p.name, p.sku into v_customer_faucet_product_name, v_customer_faucet_product_sku
      from public.products p
      where p.id = v_customer_faucet_product_id
        and lower(coalesce(p.metadata->>'product_kind','')) = 'faucet';
      if not found then raise exception 'Customer-provided Faucet catalog reference is unavailable.'; end if;
    end if;
    if v_customer_faucet_product_id is null and v_customer_faucet_note is null then
      raise exception 'Customer-provided Faucet requires a catalog match or product details.';
    end if;
    v_customer_faucet_display_name := concat_ws(' · ', 'Customer Provides', v_customer_faucet_product_name, v_customer_faucet_note);
    v_faucet_price_source := 'customer_provided';
    v_faucet := jsonb_build_object(
      'product_id', null, 'sku', v_customer_faucet_product_sku,
      'name', 'Customer Provides' || substr(v_customer_faucet_display_name, length('Customer Provides') + 1),
      'customer_provided', true,
      'customer_provided_product_id', v_customer_faucet_product_id,
      'customer_provided_product_name', v_customer_faucet_product_name,
      'customer_provided_product_sku', v_customer_faucet_product_sku,
      'customer_provided_faucet_note', v_customer_faucet_note,
      'price_source', 'customer_provided', 'unit_price', 0, 'subtotal', 0
    );
  elsif new.configuration->>'faucet_source' = 'modulex' then
    begin
      v_faucet_product_id := nullif(new.configuration->>'faucet_product_id','')::uuid;
    exception when invalid_text_representation then
      raise exception 'Faucet catalog reference is invalid.';
    end;
    if v_faucet_product_id is null then raise exception 'Modulex Faucet requires a catalog product.'; end if;
    select jsonb_build_object(
      'product_id', p.id, 'sku', p.sku, 'name', p.name,
      'commercial_price_group_id', new.price_group_id,
      'price_source', 'price_group',
      'unit_price', new.pricing_snapshot->'faucet_subtotal',
      'subtotal', new.pricing_snapshot->'faucet_subtotal'
    ) into v_faucet
    from public.products p
    where p.id = v_faucet_product_id
      and lower(coalesce(p.metadata->>'product_kind','')) = 'faucet';
    if not found then raise exception 'Faucet is unavailable.'; end if;
    v_faucet_price_source := 'price_group';
  end if;

  select coalesce(jsonb_agg(jsonb_build_object(
    'service_id', s.id, 'name', s.name, 'pricing_method', s.pricing_method,
    ...
    'subtotal', round(s.unit_price * case
  ...
  new.pricing_snapshot := new.pricing_snapshot || jsonb_build_object(
    ...
    'sink_manual_fallback', jsonb_build_object('applied', new.manual_sink_price is not null, 'amount', new.manual_sink_price),
    'faucet_price_source', v_faucet_price_source,
    'edge', v_edge,
    'sink', v_sink,
    'faucet', v_faucet,
    ...
    'totals', jsonb_build_object(
      ...
      'faucet_subtotal', coalesce(new.pricing_snapshot->'faucet_subtotal', '0'::jsonb),
