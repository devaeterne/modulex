-- PB-2 forward fix for the production-applied Project financial RPC.
-- The initial function body referenced the settings CTE as `s` without binding that alias.
-- Replacing the function is metadata-only and performs no business-data mutation.
...
comment on function public.get_customer_project_financial_summary(uuid) is
  'PB-2 read-only Project financial rollup. Runtime fallback alias fixed; cost/margin remains restricted to Super Admin, Admin, and Finance.';
