...
revoke all on function private.attach_countertop_configuration(uuid,uuid,uuid,numeric,uuid,numeric,uuid,jsonb,jsonb,numeric,numeric,text) from public,anon; grant execute on function private.attach_countertop_configuration(uuid,uuid,uuid,numeric,uuid,numeric,uuid,jsonb,jsonb,numeric,numeric,text) to authenticated;
...
-- Single reviewed mutation boundary for countertop reference management (soft-delete only).
create or replace function private.upsert_countertop_reference(p_kind text,p_id uuid default null,p_name text default null,p_code text default null,p_price numeric default null,p_pricing_method text default null,p_product_id uuid default null,p_stone_type_id uuid default null,p_material_price_band_id uuid default null,p_vendor_name text default null,p_source_ref text default null,p_is_active boolean default true) returns uuid language plpgsql security definer set search_path = pg_catalog, public as $$ declare v_id uuid; begin if not public.current_user_has_any_role(array['super_admin','admin']) then raise exception 'Countertop reference management requires admin permission.'; end if; if p_kind='stone_type' then insert into public.countertop_stone_types(id,name,is_active) values(coalesce(p_id,gen_random_uuid()),btrim(p_name),p_is_active) on conflict(id) do update set name=excluded.name,is_active=excluded.is_active,updated_at=now() returning id into v_id; elsif p_kind='material_band' then insert into public.countertop_material_price_bands(id,code,price_per_sqft,is_active,sort_order) values(coalesce(p_id,gen_random_uuid()),upper(btrim(p_code)),p_price,p_is_active,coalesce((select max(sort_order)+1 from public.countertop_material_price_bands),0)) on conflict(id) do update set code=excluded.code,price_per_sqft=excluded.price_per_sqft,is_active=excluded.is_active,updated_at=now() returning id into v_id; elsif p_kind='edge' then insert into public.countertop_edge_profiles(id,name,pricing_method,unit_price,is_active) values(coalesce(p_id,gen_random_uuid()),btrim(p_name),p_pricing_method,p_price,p_is_active) on conflict(id) do update set name=excluded.name,pricing_method=excluded.pricing_method,unit_price=excluded.unit_price,is_active=excluded.is_active,updated_at=now() returning id into v_id; elsif p_kind='service' then insert into public.countertop_services(id,name,pricing_method,unit_price,is_active) values(coalesce(p_id,gen_random_uuid()),btrim(p_name),p_pricing_method,p_price,p_is_active) on conflict(id) do update set name=excluded.name,pricing_method=excluded.pricing_method,unit_price=excluded.unit_price,is_active=excluded.is_active,updated_at=now() returning id into v_id; elsif p_kind='stone_profile' then insert into public.countertop_stone_product_profiles(product_id,stone_type_id,material_price_band_id,vendor_name,source_ref,is_active) values(p_product_id,p_stone_type_id,p_material_price_band_id,nullif(btrim(p_vendor_name),''),nullif(btrim(p_source_ref),''),p_is_active) on conflict(product_id) do update set stone_type_id=excluded.stone_type_id,material_price_band_id=excluded.material_price_band_id,vendor_name=excluded.vendor_name,source_ref=excluded.source_ref,is_active=excluded.is_active,updated_at=now() returning product_id into v_id; else raise exception 'Unknown countertop reference kind.'; end if; return v_id; end; $$;
revoke all on function private.upsert_countertop_reference(text,uuid,text,text,numeric,text,uuid,uuid,uuid,text,text,boolean) from public,anon; grant execute on function private.upsert_countertop_reference(text,uuid,text,text,numeric,text,uuid,uuid,uuid,text,text,boolean) to authenticated;
...
-- Validation-first replacement for the reference mutation boundary.
