...
create or replace function public.audit_countertop_reference_change() returns trigger language plpgsql security definer set search_path = pg_catalog, public as $$ begin insert into public.audit_logs(table_name,record_id,action,old_data,new_data,changed_by) values (tg_table_name,case when tg_table_name='countertop_stone_product_profiles' then coalesce(new.product_id,old.product_id) else coalesce(new.id,old.id) end,lower(tg_op),case when tg_op='INSERT' then null else to_jsonb(old) end,case when tg_op='DELETE' then null else to_jsonb(new) end,auth.uid()); return coalesce(new,old); end; $$;
drop trigger if exists countertop_stone_types_audit on public.countertop_stone_types; create trigger countertop_stone_types_audit after insert or update or delete on public.countertop_stone_types for each row execute function public.audit_countertop_reference_change();
drop trigger if exists countertop_material_bands_audit on public.countertop_material_price_bands; create trigger countertop_material_bands_audit after insert or update or delete on public.countertop_material_price_bands for each row execute function public.audit_countertop_reference_change();
drop trigger if exists countertop_edges_audit on public.countertop_edge_profiles; create trigger countertop_edges_audit after insert or update or delete on public.countertop_edge_profiles for each row execute function public.audit_countertop_reference_change();
drop trigger if exists countertop_services_audit on public.countertop_services; create trigger countertop_services_audit after insert or update or delete on public.countertop_services for each row execute function public.audit_countertop_reference_change();
drop trigger if exists countertop_profiles_audit on public.countertop_stone_product_profiles; create trigger countertop_profiles_audit after insert or update or delete on public.countertop_stone_product_profiles for each row execute function public.audit_countertop_reference_change();
