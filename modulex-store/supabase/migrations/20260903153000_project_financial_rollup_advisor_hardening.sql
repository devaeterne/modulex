-- PB-2 advisor hardening.
-- Keep the privileged cost/profitability implementation outside the exposed public schema,
-- and expose only a SECURITY INVOKER wrapper, matching the established Project RPC pattern.
...
comment on function private.get_customer_project_financial_summary(uuid) is
  'PB-2 privileged Project financial rollup implementation. Internal cost/margin access is guarded by application roles.';
comment on function public.get_customer_project_financial_summary(uuid) is
  'PB-2 exposed SECURITY INVOKER wrapper for the private guarded Project financial rollup implementation.';
