...
--
-- Canonical model:
--   dealer application  -> public.store_leads (lead_type = dealer_application)
--   dealer account      -> public.customers with customer_types.system_key = dealer
--   dealer login        -> public.customer_portal_users
--   lifecycle audit     -> public.store_lead_activity + public.customer_activity
--
-- Customer Documents remain private-by-default via the existing canonical document
-- lifecycle. Dealer supporting documents remain staff-only and are never projected by
-- Dealer Portal RPCs. This migration intentionally adds no parallel dealer/document tables.
...
declare
  ...
  v_customer_portal_enabled boolean;
...
begin
  ...
  if v_decision = 'reject' then
    ...
    set status = 'rejected', reviewed_by = v_actor, reviewed_at = now(), updated_by = v_actor, updated_at = now()
    where id = v_lead.id;
  ...
  -- Retry path: a completed/legacy conversion never creates a second customer.
  if v_lead.converted_customer_id is not null then
    ...
    -- Explicit linking is fail-closed: the selected customer must already be a Dealer
    -- and must match the application by normalized email or company name.
    select c.customer_code, c.name, lower(btrim(coalesce(c.email, ''))), c.status,
           c.portal_enabled, ct.system_key
      into v_customer_code, v_existing_name, v_existing_email, v_customer_status,
           v_customer_portal_enabled, v_customer_type
    ...
    if not (
      (v_email <> '' and v_existing_email = v_email)
      or (v_company <> '' and lower(btrim(coalesce(v_existing_name, ''))) = v_company)
    ) then
    ...
    v_customer_name := v_existing_name;
    ...
    -- Own approval + conversion in one transaction. The existing conversion RPC keeps
    -- duplicate matching and customer-code generation canonical.
    update public.store_leads
    set status = 'approved', reviewed_by = v_actor, reviewed_at = now(), updated_by = v_actor, updated_at = now()
    where id = v_lead.id;
    ...
    v_customer_code := v_conversion->>'customer_code';
    v_customer_name := v_conversion->>'customer_name';
    if v_customer_id is null then
      return jsonb_build_object('ok', false, 'reason', 'conversion_missing_customer');
    end if;
  end if;

  -- Validate the canonical Dealer customer before enabling access. A previously
  -- deactivated/blocked Dealer is never silently reactivated by an approval retry.
  select c.customer_code, c.name, c.status, c.portal_enabled, ct.system_key
    into v_customer_code, v_customer_name, v_customer_status, v_customer_portal_enabled, v_customer_type
  from public.customers c
  join public.customer_types ct on ct.id = c.customer_type_id and ct.is_active = true
  ...
  if v_customer_status in ('inactive', 'blocked') then
    return jsonb_build_object(
      'ok', false,
      'reason', case when v_customer_status = 'blocked' then 'dealer_account_blocked' else 'dealer_account_inactive' end,
      'customer_id', v_customer_id,
      'customer_code', v_customer_code
    );
  end if;

  if v_customer_status <> 'active' or v_customer_portal_enabled is not true then
    update public.customers
    set status = 'active', portal_enabled = true, updated_by = v_actor, updated_at = now()
    where id = v_customer_id;
  end if;

  -- Serialise portal identity preparation so retries/concurrency cannot duplicate it.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(v_customer_id::text || ':' || v_email, 0)
  );

  select cpu.id, cpu.status
    into v_portal_user_id, v_portal_user_status
  from public.customer_portal_users cpu
  where cpu.customer_id = v_customer_id
  ...
  if v_portal_user_id is null then
    select not exists (
      ...
      where cpu.customer_id = v_customer_id and cpu.is_primary = true
    ) into v_make_primary;
    ...
    insert into public.customer_activity (
      ...
      v_email,
      jsonb_build_object('portal_user_id', v_portal_user_id, 'source_lead_id', v_lead.id),
      v_actor
    );
  end if;

  -- Only record approval once. This keeps retries audit-safe.
  if not exists (
    select 1
  ...
  select
    ...
    v_reason,
    jsonb_build_object('source_lead_id', v_lead.id, 'portal_user_id', v_portal_user_id),
    v_actor
  where not exists (
    select 1
...
revoke all on function public.review_store_dealer_application(uuid,text,text,uuid) from public, anon, authenticated;
grant execute on function public.review_store_dealer_application(uuid,text,text,uuid) to authenticated;
...
declare
  ...
  v_customer_code text;
  v_customer_name text;
  v_portal_users_changed integer := 0;
begin
  if v_actor is null then
    return jsonb_build_object('ok', false, 'reason', 'not_authorized');
  end if;
  ...
  where c.id = p_customer_id and ct.is_active = true and ct.system_key = 'dealer'
  for update of c;
  ...
  if v_transition = 'deactivate' then
    ...
    get diagnostics v_portal_users_changed = row_count;
    ...
    insert into public.customer_activity (
      ...
      p_customer_id,
      'dealer_deactivated',
      'Dealer account deactivated',
      v_reason,
      jsonb_build_object(
        'transition', 'deactivate',
        'from_status', v_status,
        'to_status', 'inactive',
        'previous_portal_enabled', v_portal_enabled,
        'portal_users_suspended', v_portal_users_changed
      ),
      v_actor
    );
    ...
    return jsonb_build_object(
      ...
      'portal_users_changed', v_portal_users_changed
    );
  end if;
  ...
  insert into public.customer_activity (
    ...
    jsonb_build_object(
      ...
      'to_status', 'active',
      'previous_portal_enabled', v_portal_enabled,
      'portal_users_restored', v_portal_users_changed
    ),
    v_actor
  );
...
revoke all on function public.transition_store_dealer_account(uuid,text,text) from public, anon, authenticated;
grant execute on function public.transition_store_dealer_account(uuid,text,text) to authenticated;

-- Remove the weaker dealer-only context implementation. Dealer Portal now inherits the
-- canonical status/account-type checks from private.get_store_portal_context().
...
-- Dealer activation also requires the canonical active Dealer customer state. An invite
-- cannot activate a disabled/inactive Dealer even if an Auth session is still alive.
...
comment on column public.customer_documents.portal_visible is
  'Portal visibility is private by default and may only be changed through the canonical Admin RPC.';
comment on table public.store_lead_documents is
  'Dealer application supporting documents. Private staff-review material; never exposed by Dealer Portal RPCs.';
