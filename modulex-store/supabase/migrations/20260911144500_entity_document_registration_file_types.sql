create or replace function public.register_entity_document(
  ...
  p_file_name text,
  p_storage_path text,
  p_document_type text default 'other'::text,
...
begin
  ...
  if v_extension not in ('.pdf','.jpg','.jpeg','.png','.webp','.docx','.xls','.xlsx','.csv','.tfx') then
  ...
  if not (
    ...
    or (v_extension = '.xls' and v_mime_type = 'application/vnd.ms-excel')
    or (
      v_extension = '.xlsx'
    ...
    or (v_extension = '.tfx' and v_mime_type = 'image/tiff-fx')
  ) then
...
revoke execute on function public.register_entity_document(text, uuid, text, text, text, text, bigint, text) from public, anon;
