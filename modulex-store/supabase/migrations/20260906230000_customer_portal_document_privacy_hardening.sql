...
-- Registered Storage objects are immutable; staff may only update/delete an unregistered
-- orphan object so failed metadata registration can still clean up its upload.
...
create or replace function private.can_staff_mutate_customer_document_object(
  p_bucket_id text,
  p_object_name text
)
returns boolean
...
as $$
begin
  if auth.uid() is null or p_bucket_id <> 'customer-documents' then
    return false;
  end if;
...
revoke all on function private.can_staff_mutate_customer_document_object(text,text) from public, anon, authenticated;
...
create policy customer_documents_staff_update
on storage.objects
for update
to authenticated
using (private.can_staff_mutate_customer_document_object(bucket_id, name))
with check (private.can_staff_mutate_customer_document_object(bucket_id, name));

drop policy if exists customer_documents_staff_delete on storage.objects;
create policy customer_documents_staff_delete
on storage.objects
for delete
to authenticated
using (private.can_staff_mutate_customer_document_object(bucket_id, name));
...
begin
  ...
  if v_role not in ('super_admin', 'admin', 'sales') then raise exception 'You do not have permission to upload customer documents.' using errcode = '42501'; end if;
  if not exists (select 1 from public.customers c where c.id = p_customer_id) then raise exception 'Customer does not exist.' using errcode = '22023'; end if;
  if v_file_name is null or v_storage_path is null then raise exception 'Document file name and storage path are required.' using errcode = '22023'; end if;
  if split_part(v_storage_path, '/', 1) <> p_customer_id::text then raise exception 'Document storage path must be scoped to the customer.' using errcode = '22023'; end if;
  if p_file_size_bytes is not null and p_file_size_bytes < 0 then raise exception 'Document file size cannot be negative.' using errcode = '22023'; end if;
  ...
  insert into public.customer_documents (
    ...
    mime_type, file_size_bytes, description, is_active, portal_visible, uploaded_by
  ) values (
    p_customer_id, nullif(btrim(coalesce(p_document_type, '')), ''), v_file_name,
    'customer-documents', v_storage_path, nullif(btrim(coalesce(p_mime_type, '')), ''),
    p_file_size_bytes, nullif(btrim(coalesce(p_description, '')), ''), true, false, auth.uid()
  ) returning * into v_document;

  insert into public.customer_activity (customer_id, activity_type, title, description, metadata, actor_user_id)
  values (
    p_customer_id, 'document_uploaded', 'Customer document uploaded', v_document.file_name,
    jsonb_build_object('document_id', v_document.id, 'document_type', v_document.document_type, 'portal_visible', false),
    auth.uid()
  );
  perform set_config('modulex.customer_document_lifecycle', 'off', true);
  return v_document;
end;
$$;
...
begin
  if auth.uid() is null then raise exception 'Authentication required.' using errcode = '42501'; end if;
  select p.role into v_role from public.profiles p where p.id = auth.uid() and p.is_active = true;
  if v_role not in ('super_admin', 'admin') then raise exception 'Only Admin may change Dealer Portal document visibility.' using errcode = '42501'; end if;

  select * into v_document
  from public.customer_documents d
  where d.id = p_document_id and d.customer_id = p_customer_id and d.is_active = true
  for update;
  if v_document.id is null then raise exception 'Active customer document not found.' using errcode = '22023'; end if;
  ...
  if v_document.portal_visible is distinct from coalesce(p_visible, false) then
    ...
    insert into public.customer_activity (customer_id, activity_type, title, description, metadata, actor_user_id)
    values (
      p_customer_id, 'document_portal_visibility_changed', 'Customer document portal visibility changed', v_document.file_name,
      jsonb_build_object('document_id', v_document.id, 'portal_visible', v_document.portal_visible), auth.uid()
    );
    perform set_config('modulex.customer_document_lifecycle', 'off', true);
  end if;
  return v_document;
...
begin
  if auth.uid() is null then raise exception 'Authentication required.' using errcode = '42501'; end if;
  select p.role into v_role from public.profiles p where p.id = auth.uid() and p.is_active = true;
  if v_role not in ('super_admin', 'admin', 'sales') then raise exception 'You do not have permission to deactivate customer documents.' using errcode = '42501'; end if;
  ...
  perform set_config('modulex.customer_document_lifecycle', 'on', true);
  update public.customer_documents
  set is_active = false, portal_visible = false
  where id = v_document.id
  returning * into v_document;
  insert into public.customer_activity (customer_id, activity_type, title, description, metadata, actor_user_id)
  values (
    p_customer_id, 'document_deactivated', 'Customer document deactivated', v_document.file_name,
    jsonb_build_object('document_id', v_document.id, 'portal_visible', false), auth.uid()
  );
  perform set_config('modulex.customer_document_lifecycle', 'off', true);
  return v_document;
end;
