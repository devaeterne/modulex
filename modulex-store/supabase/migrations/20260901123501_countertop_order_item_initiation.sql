-- Sync the reviewed production countertop initiation boundary into source control.
-- This migration already exists in production as countertop_order_item_initiation.
-- It is intentionally additive and reuses the canonical Countertop pricing/attach path.
...
begin
  ...
  select p.sku, p.name into v_stone
  ...
  where p.id=p_stone_product_id and p.status='active' and pt.pricing_model='countertop_material_band';
  if not found then
    raise exception 'Stone product is unavailable or is not a canonical Countertop Material Band product.';
  end if;

  select coalesce(max(oi.line_no),0)+1 into v_line_no
  ...
  insert into public.customer_order_items(
    id,order_id,product_id,line_no,sku_snapshot,product_name_snapshot,quantity,unit_price,
    discount_percent,discount_amount,line_subtotal,line_total,price_source,countertop_reservation_quantity,created_by
  ) values(
    v_item_id,p_order_id,p_stone_product_id,v_line_no,v_stone.sku,v_stone.name,1,0,
    0,0,0,0,'price_group',p_slab_quantity,v_actor
  );

  perform private.attach_countertop_configuration(
    v_item_id,p_stone_product_id,p_price_group_id,p_sqft,p_edge_profile_id,p_edge_linear_ft,
    p_sink_product_id,p_services,p_configuration,p_manual_material_price,p_slab_quantity,p_override_reason
  );

  delete from private.countertop_order_pricing_gate
  where backend_pid=pg_backend_pid() and transaction_id=txid_current() and order_item_id=v_item_id;
