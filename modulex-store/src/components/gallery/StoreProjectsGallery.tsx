...
import type {
  StorePublicProject,
  StorePublicProjectMedia,
} from "@/lib/store/content/queries";

export type StoreProjectGalleryEntry = {
  project: StorePublicProject;
  media: StorePublicProjectMedia[];
};

type StoreProjectsGalleryProps = {
  entries: StoreProjectGalleryEntry[];
};
...
function isDirectVideoUrl(url: string) {
  return /^https?:\/\/[^\s]+\.(?:mp4|webm|ogg)(?:[?#].*)?$/i.test(url);
}
...
function getDisplayMedia(entry: StoreProjectGalleryEntry): DisplayMedia[] {
  ...
  if (isSafeImageUrl(entry.project.coverImageUrl)) {
    ...
    result.push({
      mediaType: "image",
      mediaUrl: entry.project.coverImageUrl,
      altText: entry.project.coverImageAlt,
    });
  }
...
export default function StoreProjectsGallery({ entries }: StoreProjectsGalleryProps) {
  ...
  const selected = useMemo(
    () => entries.find((entry) => entry.project.slug === selectedSlug) ?? null,
    ...
    <>
        ...
        {entries.map((entry) => (
          <div className="col-md-6 col-lg-4" key={entry.project.slug}>
            <article className="h-100 border rounded-3 overflow-hidden bg-white shadow-sm">
              ...
              >
                <img
                  src={entry.project.coverImageUrl}
                  alt={entry.project.coverImageAlt}
                  className="w-100"
                ...
                <div className="p-4">
                  ...
                  {entry.project.category || entry.project.location ? (
                    <p className="small text-muted mb-0">
                      {[entry.project.category, entry.project.location].filter(Boolean).join(" · ")}
                    </p>
                  ) : null}
                </div>
              </button>
      ...
      {selected ? (
        <div
          ...
          style={{ zIndex: 2000, background: "rgba(0, 0, 0, 0.86)", padding: "1rem" }}
          onMouseDown={(event) => {
            if (event.currentTarget === event.target) setSelectedSlug(null);
          }}
        >
          <div
            className="bg-white rounded-3 overflow-auto position-relative"
            style={{ width: "min(1100px, 100%)", maxHeight: "92vh" }}
          >
            <button
              type="button"
            ...
            <div className="p-4 p-lg-5">
              <div className="pe-5 mb-4">
                ...
                {selected.project.category || selected.project.location ? (
                  <p className="small text-muted mb-0">
                    {[selected.project.category, selected.project.location].filter(Boolean).join(" · ")}
                  </p>
                ) : null}
              </div>
                  ...
                  <div className="col-12" key={`${media.mediaUrl}-${index}`}>
                    {media.mediaType === "image" ? (
                      <img
                        ...
                        style={{ maxHeight: "75vh", objectFit: "contain", background: "#f4f4f4" }}
                      />
                    ) : isDirectVideoUrl(media.mediaUrl) ? (
                      <video className="w-100 rounded-3" controls preload="metadata" aria-label={media.altText}>
