...
export default function DealerResetPasswordForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (contextError || !context || typeof context !== "object" || !("ok" in context) || context.ok !== true) {
        throw new Error("This password reset link cannot be used here.");
      }
  ...
  if (state === "loading") {
    return <p className="text-secondary mb-0">Checking reset link…</p>;
  }

  if (state === "error" && !tokenHash) {
    return (
      <div>
        <div className="alert alert-warning" role="alert">{message}</div>
        <Link href="/dealer/forgot-password" className="small text-secondary">Request a new reset link</Link>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-3">
        <label className="form-label" htmlFor="dealer-new-password">New password</label>
        <input
      ...
      <div className="mb-4">
        <label className="form-label" htmlFor="dealer-new-password-confirm">Confirm password</label>
        <input
          id="dealer-new-password-confirm"
          className="form-control"
          type="password"
          autoComplete="new-password"
          minLength={8}
