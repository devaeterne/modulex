...
export async function dealerLoginAction(_previousState: DealerLoginState, formData: FormData): Promise<DealerLoginState> {
  ...
  if (contextError || !isStorePortalContext(context) || context.portal_kind !== "dealer") {
    await supabase.auth.signOut({ scope: "local" });
    return { error: "Dealer portal access is unavailable." };
  }
