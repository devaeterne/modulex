...
export async function POST(request: NextRequest) {
  ...
  if (!(["contact", "dealer_application"] as StoreLeadType[]).includes(leadType)) {
  ...
  if (!privacyAccepted) {
    return NextResponse.json({ error: "Privacy acknowledgement is required." }, { status: 400 });
  }
  if (leadType === "dealer_application" && !companyName) {
  ...
  const payload: StoreLeadSubmission = {
    ...
    company_website: text(body.company_website, 500),
    country_code: text(body.country_code, 2).toUpperCase(),
    city: text(body.city, 160),
    address: text(body.address, 500),
    business_type: text(body.business_type, 160),
    has_showroom: typeof body.has_showroom === "boolean" ? body.has_showroom : undefined,
    sales_channels: textArray(body.sales_channels, 20),
    estimated_annual_volume: text(body.estimated_annual_volume, 160),
    product_interests: textArray(body.product_interests, 30),
    ...
    landing_page: text(body.landing_page, 1000),
    referrer: text(body.referrer, 1000),
    website_hp: text(body.website_hp, 255),
  };

  try {
    const result = await submitStoreLead(payload);
    return NextResponse.json({ referenceCode: result.reference_code }, { status: 201 });
  } catch (error) {
