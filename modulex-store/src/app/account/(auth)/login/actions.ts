...
export async function accountLoginAction(
  ...
  const meta = claims?.claims?.app_metadata as Record<string, unknown> | undefined;
  const accountType = meta?.account_type;

  if (accountType !== "dealer_portal" && accountType !== "customer_portal") {
    await supabase.auth.signOut({ scope: "local" });
    return { error: "Account access is unavailable." };
  }

  const { data: context, error: contextError } = await supabase.rpc("get_store_portal_context");
  const expectedAccountType = isStorePortalContext(context)
    ? context.portal_kind === "dealer"
      ? "dealer_portal"
      : "customer_portal"
    : null;

  if (contextError || !isStorePortalContext(context) || expectedAccountType !== accountType) {
    await supabase.auth.signOut({ scope: "local" });
    return { error: "Account access is unavailable." };
  }
