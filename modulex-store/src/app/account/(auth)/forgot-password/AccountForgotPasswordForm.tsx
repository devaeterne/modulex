...
export default function AccountForgotPasswordForm(){
 ...
 return <form onSubmit={handleSubmit} className="portal-form">{error?<div className="portal-alert portal-alert--error" role="alert">{error}</div>:null}{message?<div className="portal-alert portal-alert--success" role="status">{message}</div>:null}<div className="portal-field"><label className="portal-label" htmlFor="account-reset-email">Email</label><input id="account-reset-email" className="portal-input" type="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} disabled={busy} required/></div><button className="portal-button portal-button--primary portal-button--full" type="submit" disabled={busy}>{busy?"Sending…":"Send reset link"}</button><div className="portal-form__footer"><Link href="/account/login" className="portal-link">Back to sign in</Link></div></form>;
}
