...
Current cross-roadmap package: **Granite Center → Oakwell GC-2D controlled production intake in progress; representative production import/review/publish, exact-SHA dedupe, unpublish and same-path republish are live-verified. A follow-up Storage audit found the private original removed through the Supabase Management API outside the Admin lifecycle while optimized/public derivatives remained valid; the verified duplicate-intake self-heal restores only missing exact private staging bytes without changing published state. Merge/deploy/re-import repair verification remains before GC-2 closeout**
...
**Cross-roadmap coordination:** Store Phase 2.1A and 2.1B are complete, and Phase 2.1C About is production-accepted. Gallery/Projects remains intentionally fail-closed until approved real Gallery/Project content is published/live-accepted. Granite GC-1, GC-2A, GC-2B, and GC-2C are complete at their package gates; GC-2D controlled production intake is in progress. The representative asset is live-published after successful Admin review, duplicate import preserved one asset/provenance row and published state, and unpublish/re-publish restored the same immutable SHA path and bytes. A later Storage audit found only the private original had been removed via the Supabase Management API outside the Admin lifecycle, leaving its DB locator dangling while optimized staging/public media stayed valid. The verified duplicate-intake self-heal checks both private staging objects, fails on integrity mismatch, and restores only missing exact bytes at the existing paths; merge/deploy/re-import repair verification remains before GC-2 closeout. Package D configurable navigation/footer remains an A4.1 obligation under the same dynamic-content rule.
