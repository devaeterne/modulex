...
async function ensureCompanyAdminCalendar(input: { ownerProfileId: string; name: string; timezone: string; actorUserId: string }) {
  const { data: owner, error: ownerError } = await supabaseAdmin.from("profiles").select("id,is_active").eq("id", input.ownerProfileId).maybeSingle();
  ...
  if (existing?.id) {
    const { error } = await supabaseAdmin.from("admin_calendars").update({
      name: input.name,
  ...
  const { data, error } = await supabaseAdmin.from("admin_calendars").insert({
    name: input.name,
    kind: "company",
    owner_profile_id: input.ownerProfileId,
    project_id: null,
...
export async function setCompanyCalendarBinding(input: {
  ...
  if (input.providerAccessRole !== "owner" && input.providerAccessRole !== "writer") throw new Error("Company Calendar requires Google writer or owner access.");
  const adminCalendarId = await ensureCompanyAdminCalendar({ ownerProfileId: input.ownerProfileId, name: input.providerCalendarName, timezone: input.timezone, actorUserId: input.actorUserId });
  ...
  if (existing && existing.binding_mode === "modulex_created") throw new Error("This Google Calendar is still bound to a legacy Project calendar.");

  let bindingId: string;
  if (existing) {
    const oldAdminCalendarId = String(existing.admin_calendar_id);
    const { data, error } = await supabaseAdmin.from("project_calendar_bindings").update({
      project_id: null,
    ...
    bindingId = String(data.id);
    if (oldAdminCalendarId !== adminCalendarId) {
      await supabaseAdmin.from("admin_calendars").update({ is_active: false, updated_by: input.actorUserId }).eq("id", oldAdminCalendarId).eq("kind", "google_imported");
    }
  } else {
    const { data, error } = await supabaseAdmin.from("project_calendar_bindings").insert({
      project_id: null,
      admin_calendar_id: adminCalendarId,
      binding_mode: "company_shared",
      provider: "google",
  ...
  const { error: settingsError } = await supabaseAdmin.from("calendar_integration_settings").update({
...
export async function getProviderEventLinkByProviderEventId(bindingId: string, providerEventId: string) {
  const { data, error } = await supabaseAdmin.from("calendar_provider_event_links").select("*").eq("provider_binding_id", bindingId).eq("provider_event_id", providerEventId).maybeSingle();
...
export async function getProviderEventLinkBySource(input: { bindingId: string; sourceType: V3SourceType; sourceId: string; occurrenceKey?: string }) {
  const { data, error } = await supabaseAdmin.from("calendar_provider_event_links").select("*")
    .eq("provider_binding_id", input.bindingId).eq("source_type", input.sourceType).eq("source_id", input.sourceId).eq("occurrence_key", input.occurrenceKey ?? "").maybeSingle();
  assertNoError(error, "Calendar provider source link could not be loaded.");
  return data as CalendarProviderEventLink | null;
}

export async function upsertProviderEventLink(input: {
  bindingId: string; sourceType: V3SourceType; sourceId: string; projectId: string | null; occurrenceKey?: string;
  providerEventId: string; providerRecurringEventId?: string | null; providerOriginalStartKey?: string | null;
  providerEtag?: string | null; providerUpdatedAt?: string | null; providerFingerprint?: string | null;
  providerObservedAt?: string | null; modulexFingerprint?: string | null; lastSyncOrigin?: "modulex" | "google" | "reconcile" | "cutover";
  syncStatus?: string; providerDeleted?: boolean;
}) {
  const row = {
  ...
  const { data, error } = await supabaseAdmin.from("calendar_provider_event_links").upsert(row, { onConflict: "provider_binding_id,source_type,source_id,occurrence_key" }).select("*").single();
...
export async function claimCalendarSyncBatch(limit = 25): Promise<CalendarSyncOutboxItem[]> {
  ...
  const { data, error } = await supabaseAdmin.from("calendar_sync_outbox").select("*")
    .in("status", ["pending", "retry"]).lte("next_attempt_at", now).order("queued_at").limit(limit);
  assertNoError(error, "Calendar sync outbox could not be loaded.");
  const rows = (data ?? []) as CalendarSyncOutboxItem[];
  if (rows.length) {
    const { error: lockError } = await supabaseAdmin.from("calendar_sync_outbox").update({ status: "processing", locked_at: now }).in("id", rows.map((row) => row.id));
...
export async function markCalendarSyncSuccess(itemId: string) {
  const { error } = await supabaseAdmin.from("calendar_sync_outbox").update({ status: "completed", completed_at: new Date().toISOString(), locked_at: null, last_error_at: null, last_error_code: null }).eq("id", itemId);
...
export async function markCalendarSyncFailure(item: CalendarSyncOutboxItem, errorCode: string) {
  ...
  const { error } = await supabaseAdmin.from("calendar_sync_outbox").update({ status: attempt >= 8 ? "error" : "retry", attempt_count: attempt, next_attempt_at: next, locked_at: null, last_error_at: new Date().toISOString(), last_error_code: errorCode }).eq("id", item.id);
  assertNoError(error, "Calendar sync failure could not be recorded.");
}

export async function enqueueCalendarSync(source: { bindingId: string; sourceType: V3SourceType; sourceId: string; projectId: string | null; operation: "upsert" | "delete"; sourceFingerprint?: string | null; occurrenceKey?: string }) {
  const { error } = await supabaseAdmin.from("calendar_sync_outbox").upsert({
    provider_binding_id: source.bindingId, source_type: source.sourceType, source_id: source.sourceId, project_id: source.projectId,
    occurrence_key: source.occurrenceKey ?? "", operation: source.operation, source_fingerprint: source.sourceFingerprint ?? null,
    status: "pending", attempt_count: 0, next_attempt_at: new Date().toISOString(), locked_at: null, queued_at: new Date().toISOString(), completed_at: null,
...
export async function enqueueCalendarSyncJob(input: { bindingId: string; jobType: "incremental" | "full" | "reconcile" | "watch_renew" | "access_check"; dedupeKey?: string; payload?: Record<string, unknown> }) {
  ...
  const { data: fallback, error: fallbackError } = await supabaseAdmin.from("calendar_sync_jobs").upsert({
    provider_binding_id: input.bindingId, job_type: input.jobType, dedupe_key: input.dedupeKey ?? "default", payload: input.payload ?? {}, status: "pending", attempt_count: 0, available_at: new Date().toISOString(), locked_at: null, completed_at: null,
  }, { onConflict: "provider_binding_id,job_type,dedupe_key" }).select("id").single();
...
export async function insertCalendarSyncAudit(input: { bindingId?: string | null; sourceType?: string | null; sourceId?: string | null; projectId?: string | null; providerEventId?: string | null; direction: "modulex_to_google" | "google_to_modulex" | "system"; action: string; resolution?: string | null; details?: Record<string, unknown>; actorProfileId?: string | null }) {
  const { error } = await supabaseAdmin.from("calendar_sync_audit").insert({
    provider_binding_id: input.bindingId ?? null, source_type: input.sourceType ?? null, source_id: input.sourceId ?? null,
    project_id: input.projectId ?? null, provider_event_id: input.providerEventId ?? null, direction: input.direction,
    action: input.action, resolution: input.resolution ?? null, details: input.details ?? {}, actor_profile_id: input.actorProfileId ?? null,
  });
...
export async function insertPendingWatchChannel(input: { bindingId: string; channelId: string; tokenHash: string; expiresAt: string | null }) {
  const { data, error } = await supabaseAdmin.from("calendar_watch_channels").insert({
    provider_binding_id: input.bindingId, channel_id: input.channelId, channel_token_hash: input.tokenHash, expires_at: input.expiresAt, status: "pending",
  }).select("*").single();
...
export async function activateWatchChannel(input: { channelId: string; resourceId: string; resourceUri: string | null; expiresAt: string | null }) {
  const { data, error } = await supabaseAdmin.from("calendar_watch_channels").update({ resource_id: input.resourceId, resource_uri: input.resourceUri, expires_at: input.expiresAt, status: "active", renewed_at: new Date().toISOString() }).eq("channel_id", input.channelId).select("*").single();
  assertNoError(error, "Calendar watch channel could not be activated.");
  return data as CalendarWatchChannelRow;
}

export async function getWatchChannel(channelId: string) {
  const { data, error } = await supabaseAdmin.from("calendar_watch_channels").select("*").eq("channel_id", channelId).maybeSingle();
...
export async function listActiveWatchChannels(bindingId: string) {
  const { data, error } = await supabaseAdmin.from("calendar_watch_channels").select("*").eq("provider_binding_id", bindingId).in("status", ["pending", "active"]).order("created_at", { ascending: false });
...
export async function markWatchMessage(channelId: string, messageNumber: number) {
  const { error } = await supabaseAdmin.from("calendar_watch_channels").update({ last_message_number: messageNumber }).eq("channel_id", channelId);
...
export async function stopWatchChannelRecord(channelId: string, status: "replaced" | "stopped" | "error" = "stopped") {
  const { error } = await supabaseAdmin.from("calendar_watch_channels").update({ status, stopped_at: new Date().toISOString() }).eq("channel_id", channelId);
