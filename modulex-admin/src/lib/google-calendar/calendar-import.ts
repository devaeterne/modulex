...
import { hasGoogleCalendarBidirectionalScopes } from "@/lib/google-calendar/config";
import {
...
function boundedFullSyncRange(now = new Date()) {
  const min = new Date(now); min.setUTCDate(min.getUTCDate() - FULL_SYNC_LOOKBACK_DAYS);
  const max = new Date(now); max.setUTCDate(max.getUTCDate() + FULL_SYNC_LOOKAHEAD_DAYS);
...
function isWriteEligible(accessRole: string | undefined) {
...
async function requireImportAccess(requestUrl?: string) {
  ...
  if (!hasGoogleCalendarBidirectionalScopes(access.grantedScopes)) {
    throw new GoogleCalendarImportError("reconnect_required", "Reconnect Google Calendar to grant calendar discovery and shared event read/write access.", 409);
  }
  return access;
}
...
export async function discoverGoogleCalendars(requestUrl?: string): Promise<GoogleCalendarDiscoveryItem[]> {
  ...
  return providerCalendars.map((calendar) => ({
    ...
    write_eligible: isWriteEligible(calendar.accessRole),
  } satisfies GoogleCalendarDiscoveryItem)).sort((a,b) => Number(b.primary)-Number(a.primary) || a.provider_calendar_name.localeCompare(b.provider_calendar_name));
}

async function assertActiveOwner(ownerProfileId: string) {
  const { data, error } = await supabaseAdmin.from("profiles").select("id,is_active").eq("id", ownerProfileId).maybeSingle();
  if (error) throw new GoogleCalendarImportError("owner_lookup_failed", "Calendar owner could not be validated.", 500);
  if (!data?.is_active) throw new GoogleCalendarImportError("invalid_owner", "Calendar owner must be an active Modulex user.", 400);
}

async function fullMirrorSync(binding: ProjectCalendarBindingRow, accessToken: string) {
  const range = boundedFullSyncRange();
  const page = await listGoogleCalendarEvents({ accessToken, calendarId: binding.provider_calendar_id, timeMin: range.timeMin, timeMax: range.timeMax });
  const events = page.items.flatMap((event) => { const normalized = normalizeGoogleEvent(event); return normalized ? [normalized] : []; });
...
export async function syncImportedGoogleCalendar(bindingId: string, requestUrl?: string) {
  ...
  if (!binding || binding.binding_mode !== "google_imported") throw new GoogleCalendarImportError("import_not_found", "Imported Google Calendar was not found.", 404);
  const access = await requireImportAccess(requestUrl);
  try {
    ...
    if (!isWriteEligible(accessRole)) {
      await markGoogleMirrorSyncError(binding.id, "provider_access_downgraded");
      throw new GoogleCalendarImportError("provider_access_downgraded", "The connected Google account no longer has writable access to this Calendar.", 409);
    }
    ...
    if (!refreshed.provider_sync_token) return await fullMirrorSync(refreshed, access.accessToken);
    try {
      const page = await listGoogleCalendarEvents({ accessToken: access.accessToken, calendarId: refreshed.provider_calendar_id, syncToken: refreshed.provider_sync_token });
      const events = page.items.flatMap((event) => { const normalized = normalizeGoogleEvent(event); return normalized ? [normalized] : []; });
    ...
    if (!(error instanceof GoogleCalendarImportError)) await markGoogleMirrorSyncError(binding.id, providerErrorCode(error));
    throw error;
  }
}

export async function importGoogleCalendar(input: { providerCalendarId: string; ownerProfileId: string; actorUserId: string; requestUrl?: string }) {
  ...
  if (!candidate) throw new GoogleCalendarImportError("provider_calendar_not_found", "Google Calendar is not available to the connected account.", 404);
  if (!candidate.write_eligible || !isWriteEligible(candidate.access_role)) {
    throw new GoogleCalendarImportError("provider_write_required", "This Calendar is visible, but the connected Google account needs writer or owner access.", 409);
  }
