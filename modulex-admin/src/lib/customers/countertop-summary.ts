...
type CountertopLineSummaryWithFaucet = CountertopLineSummary & { faucetName?: string | null; faucetSku?: string | null };
...
function parseCountertopLineSummary(row: CountertopConfigurationRow): CountertopLineSummaryWithFaucet {
  ...
  const multiSinkName = aggregateFixtures(snapshot,"sink"); const multiFaucetName = aggregateFixtures(snapshot,"faucet");
  return {
    ...
    faucetName: multiFaucetName ?? textValue(faucet.name), faucetSku: multiFaucetName ? null : textValue(faucet.sku),
...
export function formatCountertopPrintDetail(summary?: CountertopLineSummaryWithFaucet | null): string | null {
  ...
  const rows=[material.length?material.join(" · "):null,summary.edgeName?`Edge: ${summary.edgeName}${summary.edgeLinearFt!==null?` · ${compactNumber(summary.edgeLinearFt)} lf`:""}`:null,summary.sinkName?`Sink: ${summary.sinkName}${summary.sinkSku?` (${summary.sinkSku})`:""}`:null,summary.faucetName?`Faucet: ${summary.faucetName}${summary.faucetSku?` (${summary.faucetSku})`:""}`:null,summary.services.length?`Services: ${summary.services.map((service)=>`${service.name} ×${compactNumber(service.quantity)}`).join(", ")}`:null,summary.manualOverrideApplied?[summary.manualOverridePricePerSqft!==null?`Manual material price: ${compactNumber(summary.manualOverridePricePerSqft)}/sq ft`:"Manual material price override",summary.manualOverrideReason?`Reason: ${summary.manualOverrideReason}`:null].filter(Boolean).join(" · "):null].filter((value):value is string=>Boolean(value));
