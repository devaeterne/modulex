...
const REQUEST_ADMIN_EMAIL = "info@dasoft.me";
...
function requestUrl(requestId: string) {
  const base = (process.env.NEXT_PUBLIC_SITE_URL || "https://admin.oakwellcabinetry.com").replace(/\/$/, "");
...
export async function POST(request: Request) {
  ...
  const { data: supportRequest, error: requestError } = await supabaseAdmin
    ...
    .select("id,requester_id,requester_name,requester_email,title,category,description,created_at")
    .eq("id", requestId)
    .maybeSingle();
  ...
  const { data: delivery, error: deliveryError } = await supabaseAdmin
    ...
    .eq("recipient_email", REQUEST_ADMIN_EMAIL)
    .maybeSingle();

  if (deliveryError) return jsonError(deliveryError.message, 500);
  if (!delivery) return jsonError("Request email delivery was not queued.", 409);
  if (delivery.status === "sent") {
    return Response.json({ success: true, status: "sent", alreadySent: true });
  }
  ...
  try {
    ...
    if (settingsError || !settings) {
      throw new Error(settingsError?.message || "General email settings are missing.");
    }

    const senderName = String(settings.email_sender_name || settings.company_name || "Modulex").trim();
    const senderEmail = String(settings.email_sender_email || "no-reply@auth.oakwellcabinetry.com").trim();
    const requester = String(supportRequest.requester_name || supportRequest.requester_email || "Modulex user");
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        "Idempotency-Key": `support-request-created:${requestId}:${REQUEST_ADMIN_EMAIL}`.slice(0, 256),
      },
      body: JSON.stringify({
        from: `${senderName} <${senderEmail}>`,
        to: [REQUEST_ADMIN_EMAIL],
    ...
    if (!resendResponse.ok) {
      throw new Error(typeof resendPayload.message === "string" ? resendPayload.message : `Resend returned HTTP ${resendResponse.status}.`);
    }
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        status: "sent",
        attempts,
        resend_message_id: typeof resendPayload.id === "string" ? resendPayload.id : null,
        last_error: null,
    ...
    const message = errorValue instanceof Error ? errorValue.message : "Unknown request email delivery error.";
    const failedAt = new Date().toISOString();
    const nextAttemptAt = new Date(Date.now() + Math.min(60, Math.max(2, attempts * 5)) * 60_000).toISOString();
    await supabaseAdmin
      .from("support_request_email_deliveries")
      .update({
        status: "failed",
