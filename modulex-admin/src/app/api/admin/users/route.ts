...
type UserProfile = ProfileRow & { roles: UserRole[] };
...
function managedMutationError(message: string, fallbackStatus = 500) {
  if (message.includes("last_effective_super_admin")) {
    return jsonError("The last active Super Admin cannot be changed.", 409);
  }
  if (
    message.includes("users_manage permission required") ||
    message.includes("super_admin actor required") ||
    message.includes("cannot_demote_own_super_admin") ||
    message.includes("cannot_deactivate_own_account")
  ) {
...
async function getProfile(userId: string): Promise<UserProfile | null> {
  ...
  return { ...profile, roles: await getRolesForUser(profile.id, profile.role) };
}
...
async function countActiveSuperAdmins() {
  ...
  const ids = Array.from(new Set((roleRows ?? []).map((row) => String(row.user_id))));
...
async function setUserRoles(userId: string, roles: UserRole[], actorUserId: string) {
...
async function setUserAccess(
  ...
  return supabaseAdmin.rpc("admin_set_user_access", {
    target_user_id: userId,
    target_roles: roles,
...
async function handleGet(request: Request) {
  ...
  const perPage = Math.min(100, Math.max(1, Number(searchParams.get("perPage") || "100") || 100));
  ...
  if (userIds.length > 0) {
    ...
    for (const row of roleRows ?? []) {
      ...
      roleMap.set(id, [...(roleMap.get(id) ?? []), row.role as UserRole]);
    }
  }
  ...
  return Response.json({
    ...
    perPage,
    total: users.length,
    actor: { id: auth.actor.user.id, role: auth.actor.profile.role, roles: auth.actor.profile.roles },
  });
}
...
async function handlePost(request: Request) {
  ...
  if (!email || !isValidEmail(email)) return jsonError("A valid email address is required.", 400);
  if (phone && !isValidPhone(phone)) {
    return jsonError("Phone must contain 7 to 15 digits and cannot contain letters.", 400);
  }
  if (!roleResult.ok) return jsonError(roleResult.error, 400);
  ...
  if (mode === "password") {
    ...
    if (error || !data.user) return jsonError(error?.message ?? "User could not be created.", 400);
    ...
    if (error || !data.user) return jsonError(error?.message ?? "Invitation could not be sent.", 400);
    createdUserId = data.user.id;
  }
  ...
  const { error: rolesError } = await setUserRoles(createdUserId, roles, auth.actor.user.id);
  if (rolesError) {
    await supabaseAdmin.auth.admin.deleteUser(createdUserId);
    return managedMutationError(rolesError.message);
  }

  return Response.json({ user: await getProfile(createdUserId), mode }, { status: 201 });
}
...
async function handlePatch(request: Request) {
  ...
  if (!currentProfile) return jsonError("User profile was not found.", 404);
  ...
  if (action === "set_password") {
    ...
    return error ? jsonError(error.message, 400) : Response.json({ success: true });
  }
  ...
  if (action === "set_active") {
    ...
    const { error } = await setUserAccess(
      userId,
      currentProfile.roles,
      isActive,
      auth.actor.user.id
    );
    if (error) return managedMutationError(error.message);
    return Response.json({ success: true, user: await getProfile(userId) });
  }
  ...
  if (userId === auth.actor.user.id && targetIsSuperAdmin && !nextRoles.includes("super_admin")) {
  ...
  if (email !== currentProfile.email) {
    const { error: authEmailError } = await supabaseAdmin.auth.admin.updateUserById(userId, { email });
    if (authEmailError) return jsonError(authEmailError.message, 400);
  }
  ...
  const { error: accessError } = await setUserAccess(
    userId,
    nextRoles,
    currentProfile.is_active,
    auth.actor.user.id
  );
  if (accessError) return managedMutationError(accessError.message);
...
async function handleDelete(request: Request) {
  ...
  if (!userId) return jsonError("User ID is required.", 400);
  if (userId === auth.actor.user.id) return jsonError("You cannot delete your own account.", 400);
  ...
  if (currentProfile.roles.includes("super_admin")) {
    ...
    if (currentProfile.is_active && superAdminCount !== null && superAdminCount <= 1) {
      return jsonError("The last active Super Admin cannot be deleted.", 409);
    }
  }

  const { error } = await supabaseAdmin.auth.admin.deleteUser(userId);
  if (error) return managedMutationError(error.message, 400);
