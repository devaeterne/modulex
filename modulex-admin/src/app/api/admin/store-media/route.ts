...
function parseAssetId(value: unknown) {
  if (typeof value !== "string" || !UUID_PATTERN.test(value)) {
    throw new MediaApiError("A valid asset_id is required.", 400);
  }
  return value;
}
...
function verifyOptimizedBytes(asset: StoreMediaAsset, bytes: Uint8Array) {
  if (!asset.optimized_sha256 || !asset.optimized_bytes) {
    throw new MediaApiError("Optimized media metadata is incomplete.", 409);
  }
  if (bytes.byteLength !== asset.optimized_bytes) {
    throw new MediaApiError("Optimized media byte length does not match verified metadata.", 409);
  }
  if (sha256(bytes) !== asset.optimized_sha256) {
    throw new MediaApiError("Optimized media checksum does not match verified metadata.", 409);
  }
}
...
function validatePublishable(asset: StoreMediaAsset) {
  ...
  if (
    asset.staging_bucket !== STAGING_BUCKET ||
    !asset.staging_optimized_path ||
    asset.optimized_mime_type !== "image/webp" ||
    !asset.optimized_sha256 ||
...
async function publishAsset(asset: StoreMediaAsset, actorId: string) {
  if (
    ...
    asset.public_bucket === PUBLIC_BUCKET &&
    asset.public_path === `media/${asset.id}/${asset.optimized_sha256}.webp`
  ) {
  ...
  if (existingPublic) {
    verifyOptimizedBytes(asset, existingPublic);
  } else {
...
async function unpublishAsset(asset: StoreMediaAsset, actorId: string) {
  ...
  if (references.length) {
    throw new MediaApiError(`Media is still referenced by published/CMS content: ${references.join(", ")}.`, 409);
  }
  ...
  if (publicBytes) {
    ...
    if (removeError && !isMissingStorageError(removeError)) {
      throw new MediaApiError(`Unable to remove public media object: ${removeError.message}`, 502);
    }
  }
...
async function hardDeleteAsset(asset: StoreMediaAsset) {
  ...
  const references = await findReferences(asset);
  if (references.length) {
    throw new MediaApiError(`Media is still referenced by CMS content: ${references.join(", ")}.`, 409);
  }
  ...
  if (stagingPaths.length) {
    ...
    if (removeError && !isMissingStorageError(removeError)) {
      throw new MediaApiError(`Unable to remove private staging objects: ${removeError.message}`, 502);
    }
  }
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (body.action !== "publish" && body.action !== "unpublish") {
      throw new MediaApiError("Action must be publish or unpublish.", 400);
    }

    const asset = await loadAsset(assetId);
    const updated = body.action === "publish"
      ? await publishAsset(asset, authorization.actor.user.id)
      : await unpublishAsset(asset, authorization.actor.user.id);
