...
export default function PersonnelOverview(){
 ...
 return <div className="space-y-6"><div className={card}><h1 className="text-2xl font-semibold text-gray-800 dark:text-white/90">Personnel & HR</h1><p className="mt-2 max-w-4xl text-sm text-gray-500 dark:text-gray-400">Central employee lifecycle management from hire through attendance, leave, compensation, payroll, benefits, compliance, performance and offboarding.</p></div>{error&&<div role="alert" className="rounded-xl border border-error-200 bg-error-50 px-4 py-3 text-sm text-error-700 dark:border-error-500/30 dark:bg-error-500/10 dark:text-error-300">{error}</div>}<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-busy={loading}>{cards.map(([label,value,note])=><div key={String(label)} className={card}><p className="text-sm text-gray-500">{label}</p><p className="mt-2 text-2xl font-semibold text-gray-800 dark:text-white/90" aria-live="polite">{loading?"—":value}</p><p className="mt-1 text-xs text-gray-400">{note}</p></div>)}</div><nav aria-label="Personnel modules" className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{modules.map(([href,title,description])=><Link key={href} href={href} className={card+" transition hover:border-brand-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 dark:hover:border-brand-500/50"}><h2 className="font-semibold text-gray-800 dark:text-white/90">{title}</h2><p className="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">{description}</p><span className="mt-4 inline-flex text-sm font-medium text-brand-500">Open →</span></Link>)}</nav></div>;
}
