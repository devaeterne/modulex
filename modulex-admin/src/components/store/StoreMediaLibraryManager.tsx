  ...
  const createPreviewUrl = useCallback(async (asset: StoreMediaAsset) => {
    ...
    if (asset.staging_bucket === "store-media-staging" && asset.staging_optimized_path) {
      const { data, error: signedError } = await supabase.storage
        .from(asset.staging_bucket)
        .createSignedUrl(asset.staging_optimized_path, 300);
  ...
  const load = useCallback(async () => {
    ...
    if (assetResult.error) setError(assetResult.error.message);
      ...
      const previewEntries = await Promise.all(
        nextAssets.map(async (asset) => [asset.id, await createPreviewUrl(asset)] as const),
      );
      setPreviewUrls(
        Object.fromEntries(
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
      <aside className="space-y-5">
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Review staged assets and control publication without exposing Granite source URLs as runtime media.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 space-y-3">
            <input
              ...
              placeholder="Search title, candidate or source"
            />
        ...
        <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
          {filteredAssets.length === 0 ? (
            <p className="p-5 text-sm text-gray-500">No media assets match the current filters.</p>
          ) : null}
          {filteredAssets.map((asset) => {
            const provenance = sourcesByAsset.get(asset.id) ?? [];
            const summary =
              provenance[0]?.source_candidate_id ??
              provenance[0]?.source_label ??
              provenance[0]?.source_brand ??
              provenance[0]?.source_site ??
              "No provenance";
            ...
            return (
              <button
                ...
                className={`block w-full border-b border-gray-100 p-4 text-left last:border-b-0 dark:border-gray-800 ${selectedId === asset.id ? "bg-brand-50/60 dark:bg-brand-500/10" : "hover:bg-gray-50 dark:hover:bg-white/[0.02]"}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-2 text-xs text-gray-400">
                      {formatMediaDimensions(
                        asset.optimized_width ?? asset.original_width,
                        asset.optimized_height ?? asset.original_height,
                      )}{" "}
                      · {formatMediaBytes(asset.optimized_bytes ?? asset.original_bytes)}
                    </p>
      ...
      <main className="min-w-0">
        {selected ? (
          <StoreMediaAssetEditor
            asset={selected}
            sources={selectedSources}
            canEdit={canEdit}
            publicPreviewUrl={selectedPreviewUrl}
            onSaved={handleSaved}
            onDeleted={handleDeleted}
          />
        ) : (
          <div className="rounded-2xl border border-gray-200 bg-white p-8 text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-900">
            Select a media asset to review it.
          </div>
        )}
      </main>
    </div>
