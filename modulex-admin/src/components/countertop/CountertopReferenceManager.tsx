...
type PriceEntryMode = "fixed" | "manual";
type ReferenceRow = {
  id: string;
  name?: string | null;
  code?: string | null;
...
type CountertopReferenceManagerProps = { kinds?: readonly ReferenceKind[] };

const EMPTY_DRAFT: ReferenceDraft = { name: "", code: "", price: "", pricing_method: "", price_entry_mode: "fixed" };
const METHOD_OPTIONS = [
  { value: "each", label: "Each" }, { value: "sq_ft", label: "Sq ft" },
  { value: "linear_ft", label: "Linear ft" }, { value: "flat", label: "Flat" },
];
const MODE_OPTIONS = [
  { value: "fixed", label: "Fixed Price" },
  { value: "manual", label: "Manual Per Order" },
];
...
const CONFIGS: ReferenceConfig[] = [
  ...
  { key: "service", title: "Additional Services", description: "Manage fixed-price and Manual Per Order services available in Countertop configuration.", table: "countertop_services", columns: "id,name,pricing_method,unit_price,price_entry_mode,is_active", orderBy: "name" },
];
...
export default function CountertopReferenceManager({ kinds }: CountertopReferenceManagerProps = {}) {
  ...
  const load = useCallback(async () => {
    ...
    if (failed?.[1].error) { setError(failed[1].error.message || "Unable to load Countertop references."); setLoading(false); return false; }
    ...
    setRows(next); setLoading(false); return true;
  }, [visibleConfigs]);
  ...
  function openEdit(kind: ReferenceKind, row: ReferenceRow) {
    ...
    setDraft({
      ...
      price: row.price_entry_mode === "manual" ? "" : String(row.price_per_sqft ?? row.unit_price ?? ""),
      pricing_method: row.pricing_method ?? "", price_entry_mode: row.price_entry_mode ?? "fixed",
    });
    setEditor(kind);
  }
  function closeEditor() { if (saving && saving !== "status") return; setEditor(null); setDraft(EMPTY_DRAFT); }
  ...
  function validateDraft(kind: ReferenceKind) {
    ...
    if (kind === "edge" && (!draft.name.trim() || !draft.pricing_method || draft.price.trim() === "")) return "Name, pricing method and unit price are required.";
    if (kind === "service" && (!draft.name.trim() || !draft.pricing_method)) return "Name and pricing method are required.";
    if (kind === "service" && draft.price_entry_mode === "fixed" && draft.price.trim() === "") return "Fixed Price service requires a unit price.";
    if (kind !== "stone_type" && draft.price.trim() !== "" && (!Number.isFinite(Number(draft.price)) || Number(draft.price) < 0)) return "Price must be a non-negative number.";
    return null;
  }
  ...
  async function saveReference() {
    ...
    setSaving(editor);
    const response = editor === "service"
      ? await supabase.rpc("upsert_countertop_service_reference", {
          p_id: draft.id ?? null,
          p_name: draft.name.trim(),
          p_pricing_method: draft.pricing_method,
          p_unit_price: draft.price_entry_mode === "fixed" ? draft.price : null,
          p_price_entry_mode: draft.price_entry_mode,
          p_is_active: true,
        })
      : await supabase.rpc("upsert_countertop_reference", {
          p_kind: editor, p_id: draft.id ?? null, p_name: draft.name.trim() || null,
          p_code: draft.code.trim().toUpperCase() || null,
          p_price: editor === "stone_type" ? null : draft.price.trim(),
          p_pricing_method: editor === "edge" ? draft.pricing_method : null, p_is_active: true,
        });
    setSaving(null);
    if (response.error) { setError(response.error.message.includes("already exists") ? "Duplicate reference." : response.error.message); return; }
    const successMessage = `${activeConfig?.title ?? "Countertop reference"} saved.`;
    setEditor(null); setDraft(EMPTY_DRAFT); if (await load()) setMessage(successMessage);
  }

  async function toggleReference(config: ReferenceConfig, row: ReferenceRow) {
    resetMessages(); setSaving("status");
    const response = config.key === "service"
      ? await supabase.rpc("upsert_countertop_service_reference", {
          p_id: row.id, p_name: row.name ?? null, p_pricing_method: row.pricing_method ?? "each",
          p_unit_price: row.unit_price ?? 0, p_price_entry_mode: row.price_entry_mode ?? "fixed", p_is_active: !row.is_active,
        })
      : await supabase.rpc("upsert_countertop_reference", {
          p_kind: config.key, p_id: row.id, p_name: row.name ?? null, p_code: row.code ?? null,
          p_price: row.price_per_sqft ?? row.unit_price ?? null, p_pricing_method: row.pricing_method ?? null, p_is_active: !row.is_active,
        });
    setSaving(null);
  ...
  function referencePrice(config: ReferenceConfig, row: ReferenceRow) {
    ...
    if (config.key === "service" && row.price_entry_mode === "manual") return "Manual Per Order";
  ...
  function referenceMethod(config: ReferenceConfig, row: ReferenceRow) {
    ...
    if (config.key === "stone_type") return "—";
    const method = row.pricing_method ? (METHOD_LABELS[row.pricing_method] ?? row.pricing_method) : "—";
    return config.key === "service" ? `${method} · ${row.price_entry_mode === "manual" ? "Manual Per Order" : "Fixed Price"}` : method;
  }
  ...
  return <div className="space-y-6">
    ...
    {visibleConfigs.map((config) => <ComponentCard key={config.key} title={config.title} desc={config.description} headerAction={<Button onClick={() => openNew(config.key)}>Add</Button>}>
      <TableViewport><Table variant="admin" minWidth="standard"><TableHeader variant="admin"><TableRow>
        ...
        {loading ? <TableStateRow colSpan={5}>Loading {config.title}…</TableStateRow> : rows[config.key].length === 0 ? <TableStateRow colSpan={5}>No references.</TableStateRow> : rows[config.key].map((row) => <TableRow key={row.id}>
    ...
    <Modal isOpen={editor !== null} onClose={closeEditor} className="m-4 max-h-[90vh] max-w-2xl overflow-y-auto p-6" ariaLabel="Countertop reference editor">
      {activeConfig ? <div className="space-y-6"><h3 className="text-lg font-semibold">{draft.id ? `Edit ${activeConfig.title}` : `Add ${activeConfig.title}`}</h3>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {editor === "service" ? <div><Label htmlFor="reference-price-mode">Price Entry Mode *</Label><Select id="reference-price-mode" value={draft.price_entry_mode} options={MODE_OPTIONS} onChange={(value) => setDraft((current) => ({...current,price_entry_mode:value as PriceEntryMode,price:value === "manual" ? "" : current.price}))} /></div> : null}
          {editor !== "stone_type" && !(editor === "service" && draft.price_entry_mode === "manual") ? <div><Label htmlFor="reference-price">{editor === "material_band" ? "Price / sq ft *" : "Unit Price *"}</Label><Input id="reference-price" type="number" min={0} step="0.01" value={draft.price} onChange={(event) => setDraft((current) => ({...current,price:event.target.value}))} /></div> : null}
