...
export default function VendorBillDetailPanel({ invoiceId, canManage, onChanged }: { invoiceId: string; canManage: boolean; onChanged: () => Promise<void> | void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "Audit" ? <div className="space-y-2">{detail.audit.length === 0 ? <p className={ADMIN_TEXT_STYLES.muted}>No Vendor Bill audit events.</p> : detail.audit.map((row, index) => <div key={text(row.id) || String(index)} className="rounded-lg border border-gray-200 p-3 dark:border-gray-800"><div className="font-medium">{text(row.action_type) || "Vendor Bill event"}</div><div className={`text-xs ${ADMIN_TEXT_STYLES.muted}`}>{text(row.created_at)}</div>{text(row.reason) ? <div className="mt-1 text-sm">{text(row.reason)}</div> : null}</div>)}</div> : null}
    </div>
  );
}