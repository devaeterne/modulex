...
export default function FinanceVendorsManager() {
  ...
  const [detailTab, setDetailTab] = useState<VendorDetailTab>("Overview");
  ...
  async function loadVendorList(nextOffset = offset, nextSearch = search, nextStatus = statusFilter, nextType = vendorTypeFilter) {
    setLoading(true);
    try {
      const nextVendors = await getVendorsPage({
        limit: pageSize,
        offset: nextOffset,
        status: (nextStatus || null) as VendorStatus | null,
        search: nextSearch,
        vendorType: (nextType || null) as VendorType | null,
      });
  ...
  async function loadDetail(vendorId: string, preserveTab = false) {
    setDetailLoading(true);
    try {
      const nextDetail = await getVendorDetail(vendorId);
      setSelectedId(vendorId);
      setDetail(nextDetail);
      if (!preserveTab) setDetailTab("Overview");
    } catch (error) {
      setMessage({ variant: "error", text: error instanceof Error ? error.message : "Vendor detail could not be loaded." });
    } finally {
  ...
  async function refreshSelected() {
    await loadVendorList(offset);
    if (selectedId) await loadDetail(selectedId, true);
  }

  useEffect(() => {
    let active = true;
    Promise.all([getCurrentProfile(), getVendorSourceCandidates()])
      .then(([profileResult, nextSources]) => {
        if (!active) return;
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      void loadVendorList(offset, search, statusFilter, vendorTypeFilter);
    }, 250);
    return () => window.clearTimeout(timer);
    // Filter state intentionally owns Vendor list refresh.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [offset, search, statusFilter, vendorTypeFilter]);
