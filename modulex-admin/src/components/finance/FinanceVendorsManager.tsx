...
export default function FinanceVendorsManager() {
  ...
  const [complianceStatus, setComplianceStatus] = useState<VendorComplianceStatus>("pending");
  ...
  async function load(nextOffset = offset, nextSearch = search, nextStatus = statusFilter) {
    setLoading(true);
    try {
      ...
      const [nextVendors, nextSources] = await Promise.all([
        getVendorsPage({
          limit: pageSize,
          offset: nextOffset,
          status: (nextStatus || null) as VendorStatus | null,
          search: nextSearch,
        }),
        getVendorSourceCandidates(),
      ]);
  ...
  async function submitVendor(event: FormEvent) {
    ...
    try {
      ...
      if (editingVendor && selectedId) {
        ...
        setMessage({ variant: "success", text: "Vendor created in onboarding status. Source identities stay unmapped until explicitly linked." });
      }
      resetVendorForm();
      await load(0);
  ...
  async function submitCompliance(event: FormEvent) {
    ...
    try {
      ...
      setFileName("");
      setComplianceNotes("");
      setMessage({ variant: "success", text: "Compliance metadata saved. Missing/expired compliance stays a warning and does not block payment." });
      await refreshSelected();
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <ComponentCard title="Vendor Overview" desc="One canonical AP identity; inactive vendors remain readable for historical procurement, invoice and Finance references.">
        <div className="space-y-4">
              ...
              <TableBody>
                {loading ? <TableStateRow colSpan={6}>Loading vendors…</TableStateRow> : vendors.length === 0 ? <TableStateRow colSpan={6}>No canonical vendors match these filters.</TableStateRow> : vendors.map((vendor) => (
                  <TableRow key={vendor.id}>
                    <TableCell><div className={`font-medium ${ADMIN_TEXT_STYLES.strong}`}>{vendor.display_name}</div><div className={`text-xs ${ADMIN_TEXT_STYLES.muted}`}>{vendor.code} · {vendor.legal_name}</div></TableCell>
          ...
          <div className={`flex flex-wrap items-center justify-between gap-3 text-sm ${ADMIN_TEXT_STYLES.muted}`}><span>{totalCount} vendor{totalCount === 1 ? "" : "s"}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={loading || offset === 0} onClick={() => void load(Math.max(0, offset - pageSize))}>Previous</Button><Button size="sm" variant="outline" disabled={loading || offset + pageSize >= totalCount} onClick={() => void load(offset + pageSize)}>Next</Button></div></div>
        </div>
      </ComponentCard>
        ...
        <>
          ...
          <ComponentCard title={`${detail.vendor.display_name} · Vendor Detail`} desc="Canonical business identity, remittance profile and lifecycle.">
            <div className="grid gap-4 text-sm md:grid-cols-2 xl:grid-cols-4">
              ...
              <div><span className={ADMIN_TEXT_STYLES.muted}>Status</span><div><Badge color={statusColor(detail.vendor.status)}>{detail.vendor.status}</Badge></div></div>
          ...
          <ComponentCard title="Source Identities" desc="Source mapping is explicit. Vendor Catalog codes stay integration identities until linked here.">
            {canManage ? <form onSubmit={submitSource} className="mb-5 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-end"><div><Label htmlFor="vendor-source">Source candidate</Label><Select id="vendor-source" options={sourceOptions} value={sourceChoice} onChange={setSourceChoice} placeholder="Choose a catalog/procurement/invoice source" /></div><Checkbox id="vendor-source-primary" label="Primary Source" checked={sourcePrimary} onChange={setSourcePrimary} disabled={busy} /><Button type="submit" disabled={busy || !sourceChoice}>Map Source</Button></form> : null}
