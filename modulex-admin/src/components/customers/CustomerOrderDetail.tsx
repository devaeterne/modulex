...
const selectClass = "h-10 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30";

function money(value: string | number | null | undefined, currency = "USD") { const n = Number(value ?? 0); try { return new Intl.NumberFormat("en-US", { style: "currency", currency }).format(Number.isFinite(n) ? n : 0); } catch { return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(Number.isFinite(n) ? n : 0); } }
...
export default function CustomerOrderDetail() {
  ...
  if (!customer || !order) return <div className="rounded-2xl border border-error-200 bg-error-50 p-6 text-error-700 dark:border-error-500/30 dark:bg-error-500/10 dark:text-error-400">{errorMessage || "Order not found."}</div>;
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold text-gray-800 dark:text-white/90">{order.order_number}</h1><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(order.status)}`}>{titleCase(order.status)}</span></div><p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{customer.name} • {customer.customer_code} • {date(order.order_date)}</p></div><div className="flex flex-wrap gap-2"><Link href={`/customers/${customer.id}/orders/${order.id}/print`} target="_blank" className="inline-flex h-10 items-center rounded-lg bg-brand-500 px-4 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">Print Order</Link><Link href={`/customers/${customer.id}`} className="inline-flex h-10 items-center rounded-lg border border-gray-300 bg-white px-4 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-transparent dark:text-gray-300 dark:hover:bg-white/[0.05]">Customer Card</Link><Link href={`/customers/${customer.id}/orders`} className="inline-flex h-10 items-center rounded-lg border border-gray-300 bg-white px-4 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-transparent dark:text-gray-300 dark:hover:bg-white/[0.05]">All Customer Orders</Link></div></div>
    </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"><div className="border-b border-gray-200 px-5 py-4 dark:border-gray-800"><h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">Order Items</h2></div><div className="overflow-x-auto"><table className="min-w-full divide-y divide-gray-100 dark:divide-gray-800"><thead className="bg-gray-50 dark:bg-white/[0.02]"><tr>{["#","SKU","Product","Qty","Unit Price","Discount","Total","Source"].map((l) => <th key={l} className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{l}</th>)}</tr></thead><tbody className="divide-y divide-gray-100 dark:divide-gray-800">{items.map((item) => <tr key={item.id}><td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400">{item.line_no}</td><td className="px-4 py-4 text-sm font-semibold text-gray-800 dark:text-white/90">{item.sku_snapshot}</td><td className="px-4 py-4 text-sm text-gray-700 dark:text-gray-300">{item.product_name_snapshot}</td><td className="px-4 py-4 text-sm text-gray-700 dark:text-gray-300">{Number(item.quantity)}</td><td className="px-4 py-4 text-sm text-gray-700 dark:text-gray-300">{money(item.unit_price, currency)}</td><td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400">{Number(item.discount_percent).toFixed(1)}%</td><td className="px-4 py-4 text-sm font-semibold text-gray-800 dark:text-white/90">{money(item.line_total, currency)}</td><td className="px-4 py-4"><span className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-600 dark:bg-white/[0.06] dark:text-gray-400">{titleCase(item.price_source)}</span></td></tr>)}</tbody></table></div></div>

    <div className="grid gap-5 xl:grid-cols-12">
      <div className="space-y-5 xl:col-span-8"><div className="grid gap-5 md:grid-cols-2"><AddressCard title="Billing Address" data={billing} /><AddressCard title="Shipping Address" data={shipping} /></div><div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]"><h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">Notes & References</h2><div className="mt-4 grid gap-4 md:grid-cols-3"><Info label="Customer Reference" value={order.customer_reference} /><Info label="Customer Notes" value={order.customer_notes} /><Info label="Internal Notes" value={order.internal_notes} /></div></div></div>
      <div className="space-y-5 xl:col-span-4"><div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]"><h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">Totals & Payment</h2><div className="mt-4 space-y-3 text-sm"><Row label="Subtotal" value={money(order.subtotal, currency)} /><Row label="Order Discount" value={`-${money(order.discount_amount, currency)}`} /><Row label={`Tax (${Number(order.tax_rate).toFixed(1)}%)`} value={money(order.tax_amount, currency)} /><Row label="Order Total" value={money(order.total_amount, currency)} /><Row label="Payment Method" value={order.payment_method_name_snapshot || "—"} />{Number(order.payment_commission_amount ?? 0) > 0 && <Row label={`Commission (${Number(order.payment_commission_percent).toFixed(2)}%)`} value={money(order.payment_commission_amount, currency)} />}<div className="border-t border-gray-200 pt-3 dark:border-gray-800"><Row label="Grand Total" value={money(grandTotal, currency)} strong /></div></div></div><div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]"><h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">Status Timeline</h2><div className="mt-4 space-y-4">{history.map((entry) => <div key={entry.id} className="border-l-2 border-brand-200 pl-3"><p className="text-sm font-medium text-gray-800 dark:text-white/90">{titleCase(entry.to_status)}</p><p className="mt-0.5 text-xs text-gray-400">{dateTime(entry.created_at)}{entry.changed_by ? ` • ${profileMap.get(entry.changed_by) ?? "User"}` : ""}</p>{entry.note && <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{entry.note}</p>}</div>)}</div></div></div>
    </div>
  </div>;
}

function Summary({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]"><p className="text-xs text-gray-500 dark:text-gray-400">{label}</p><p className="mt-1 text-lg font-semibold text-gray-800 dark:text-white/90">{value}</p></div>; }
function Row({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) { return <div className="flex items-center justify-between gap-4"><span className="text-gray-500 dark:text-gray-400">{label}</span><span className={strong ? "text-lg font-semibold text-gray-800 dark:text-white/90" : "font-medium text-gray-800 dark:text-white/90"}>{value}</span></div>; }
