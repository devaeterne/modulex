...
const textareaClass =
  "min-h-[100px] w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800 shadow-theme-xs transition placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90";
...
const dangerButtonClass =
  "inline-flex h-9 items-center justify-center rounded-lg border border-error-200 bg-error-50 px-3 text-xs font-medium text-error-700 transition hover:bg-error-100 dark:border-error-500/30 dark:bg-error-500/10 dark:text-error-400";
...
export default function CustomerCard() {
  ...
  async function removeContact(id: string) {
    ...
    await logActivity("contact_removed", "Contact removed"); await loadData();
  }

  async function addAddress() {
    if (!addressForm.address_name.trim() || !addressForm.address_line_1.trim() || !addressForm.city.trim() || addressForm.country_code.trim().length !== 2) {
      return setErrorMessage("Address name, address line, city and 2-letter country code are required.");
    }
    clearMessages(); setIsSaving(true);
  ...
  async function removeAddress(id: string) {
    ...
    if (error) return setErrorMessage(error.message);
    await logActivity("address_removed", "Address removed"); await loadData();
  }

  async function saveCommercial() {
    if (!commercial) return;
    clearMessages(); setIsSaving(true);
    const { error } = await supabase.from("customer_commercial_settings").upsert(commercial, { onConflict: "customer_id" });
    if (error) { setErrorMessage(error.message); setIsSaving(false); return; }
    await logActivity("commercial_updated", "Commercial settings updated"); setSuccessMessage("Commercial settings saved."); setIsSaving(false);
  }
  ...
  async function removePortalUser(id: string) {
    if (!canManagePortal || !confirm("Remove this portal user?")) return;
    const { error } = await supabase.from("customer_portal_users").delete().eq("id", id);
    if (error) return setErrorMessage(error.message);
    await logActivity("portal_user_removed", "Portal user removed"); await loadData();
  }
  ...
  async function removeNote(id: string) {
    ...
    if (error) return setErrorMessage(error.message);
    await logActivity("note_removed", "Customer note removed"); await loadData();
  }
  ...
  if (!customer || errorMessage && !customer) {
    return <div className="rounded-2xl border border-error-200 bg-error-50 p-6 text-error-700 dark:border-error-500/30 dark:bg-error-500/10 dark:text-error-400">{errorMessage || "Customer not found."}</div>;
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl font-semibold text-gray-800 dark:text-white/90">{customer.name}</h1>
              <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(customer.status)}`}>{titleCase(customer.status)}</span>
            </div>
            <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-gray-500 dark:text-gray-400">
              <span>{customer.customer_code}</span>
              <span>{customer.customer_type_id ? typeMap.get(customer.customer_type_id) ?? "—" : "—"}</span>
              <span>{customer.price_group_id ? groupMap.get(customer.price_group_id) ?? "—" : "—"}</span>
              <span>{customer.country_code || "No country"}</span>
              <span>Portal: {customer.portal_enabled ? "Enabled" : "Disabled"}</span>
            </div>
          </div>
          <Link href="/customers" className={secondaryButtonClass}>Back to Customers</Link>
        </div>

        <div className="mt-6 overflow-x-auto border-t border-gray-100 pt-4 dark:border-gray-800">
          <div className="flex min-w-max gap-1">
            {tabs.map((tab) => (
              <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`rounded-lg px-3 py-2 text-sm font-medium transition ${activeTab === tab ? "bg-brand-50 text-brand-700 dark:bg-brand-500/10 dark:text-brand-400" : "text-gray-500 hover:bg-gray-50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.04] dark:hover:text-white"}`}>{tab}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {activeTab === "General" && (
        <Section title="General Information" description="Core customer identity and account status.">
          ...
          {canEdit && <div className="mt-5 flex justify-end"><button disabled={isSaving || !customer.name.trim()} onClick={() => saveCustomer({ name: customer.name.trim(), legal_name: customer.legal_name, customer_type_id: customer.customer_type_id, status: customer.status, tax_number: customer.tax_number, registration_number: customer.registration_number, email: customer.email, phone: customer.phone, website: customer.website, country_code: customer.country_code, language_code: customer.language_code, currency_code: customer.currency_code, sales_rep_id: customer.sales_rep_id, customer_since: customer.customer_since }, "General customer information updated")} className={primaryButtonClass}>{isSaving ? "Saving..." : "Save General"}</button></div>}
        </Section>
      )}

      {activeTab === "Contacts" && (
        <Section title="Contacts" description="People associated with this customer account.">
          <div className="grid gap-3 lg:grid-cols-2">
            {contacts.map((contact) => <Card key={contact.id}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-gray-800 dark:text-white/90">{contact.first_name} {contact.last_name}</h3>{contact.is_primary && <Badge>Primary</Badge>}</div><p className="mt-1 text-sm text-gray-500">{[contact.job_title, contact.department].filter(Boolean).join(" • ") || "No role"}</p><p className="mt-3 text-sm text-gray-600 dark:text-gray-300">{contact.email || "—"}</p><p className="text-sm text-gray-500">{contact.mobile || contact.phone || "—"}</p><div className="mt-3 flex flex-wrap gap-1">{contact.is_billing_contact && <Badge>Billing</Badge>}{contact.is_shipping_contact && <Badge>Shipping</Badge>}{contact.is_order_contact && <Badge>Orders</Badge>}</div></div>{canEdit && <button onClick={() => removeContact(contact.id)} className={dangerButtonClass}>Remove</button>}</div></Card>)}
          </div>
          {canEdit && <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><h3 className="mb-4 text-sm font-semibold text-gray-800 dark:text-white/90">Add Contact</h3><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{(["first_name", "last_name", "job_title", "department", "email", "phone", "mobile"] as const).map((key) => <Field key={key} label={titleCase(key)}><input value={contactForm[key]} onChange={(e) => setContactForm({ ...contactForm, [key]: e.target.value })} className={inputClass} /></Field>)}</div><div className="mt-4 flex flex-wrap gap-4"><Check label="Primary" checked={contactForm.is_primary} onChange={(v) => setContactForm({ ...contactForm, is_primary: v })} /><Check label="Billing" checked={contactForm.is_billing_contact} onChange={(v) => setContactForm({ ...contactForm, is_billing_contact: v })} /><Check label="Shipping" checked={contactForm.is_shipping_contact} onChange={(v) => setContactForm({ ...contactForm, is_shipping_contact: v })} /><Check label="Orders" checked={contactForm.is_order_contact} onChange={(v) => setContactForm({ ...contactForm, is_order_contact: v })} /></div><div className="mt-4 flex justify-end"><button onClick={addContact} disabled={isSaving} className={primaryButtonClass}>Add Contact</button></div></div>}
        </Section>
      )}

      {activeTab === "Pricing" && (
        <Section title="Pricing" description="Customer-specific price group assignment.">
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Default Price Group"><select value={customer.price_group_id ?? ""} disabled={!canEdit} onChange={(e) => setCustomer({ ...customer, price_group_id: e.target.value || null })} className={inputClass}><option value="">No price group</option>{priceGroups.map((item) => <option key={item.id} value={item.id}>{item.name}{item.is_base_price ? " (Base)" : ""}</option>)}</select></Field>
            <Field label="Currency"><input value={customer.currency_code} disabled={!canEdit} onChange={(e) => setCustomer({ ...customer, currency_code: e.target.value.toUpperCase() })} className={inputClass} /></Field>
          </div>
          <div className="mt-4 rounded-xl border border-brand-100 bg-brand-25 p-4 text-sm text-gray-600 dark:border-brand-500/20 dark:bg-brand-500/[0.06] dark:text-gray-300">Store pricing will resolve from this customer&apos;s assigned price group. Cost and margin data are intentionally not exposed here.</div>
          {canEdit && <div className="mt-5 flex justify-end"><button onClick={() => saveCustomer({ price_group_id: customer.price_group_id, currency_code: customer.currency_code }, "Customer pricing assignment updated")} className={primaryButtonClass}>Save Pricing</button></div>}
        </Section>
      )}

      {activeTab === "Addresses" && (
        <Section title="Billing & Shipping Addresses" description="Multiple operational addresses with billing and shipping defaults.">
          <div className="grid gap-3 lg:grid-cols-2">{addresses.map((address) => <Card key={address.id}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-gray-800 dark:text-white/90">{address.address_name}</h3><Badge>{titleCase(address.address_type)}</Badge>{address.is_default_billing && <Badge>Default Billing</Badge>}{address.is_default_shipping && <Badge>Default Shipping</Badge>}</div><p className="mt-3 text-sm text-gray-700 dark:text-gray-300">{address.address_line_1}{address.address_line_2 ? `, ${address.address_line_2}` : ""}</p><p className="text-sm text-gray-500">{[address.postal_code, address.city, address.state_region, address.country_code].filter(Boolean).join(", ")}</p><p className="mt-2 text-xs text-gray-400">{address.contact_name || address.phone || ""}</p></div>{canEdit && <button onClick={() => removeAddress(address.id)} className={dangerButtonClass}>Remove</button>}</div></Card>)}</div>
          {canEdit && <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><h3 className="mb-4 text-sm font-semibold text-gray-800 dark:text-white/90">Add Address</h3><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{(["address_name", "company_name", "contact_name", "address_line_1", "address_line_2", "postal_code", "city", "state_region", "country_code", "phone"] as const).map((key) => <Field key={key} label={titleCase(key)}><input value={addressForm[key]} maxLength={key === "country_code" ? 2 : undefined} onChange={(e) => setAddressForm({ ...addressForm, [key]: key === "country_code" ? e.target.value.toUpperCase() : e.target.value })} className={inputClass} /></Field>)}<Field label="Address Type"><select value={addressForm.address_type} onChange={(e) => setAddressForm({ ...addressForm, address_type: e.target.value as "billing" | "shipping" | "both" })} className={inputClass}><option value="billing">Billing</option><option value="shipping">Shipping</option><option value="both">Both</option></select></Field></div><div className="mt-4 flex gap-4"><Check label="Default Billing" checked={addressForm.is_default_billing} onChange={(v) => setAddressForm({ ...addressForm, is_default_billing: v })} /><Check label="Default Shipping" checked={addressForm.is_default_shipping} onChange={(v) => setAddressForm({ ...addressForm, is_default_shipping: v })} /></div><div className="mt-4 flex justify-end"><button onClick={addAddress} disabled={isSaving} className={primaryButtonClass}>Add Address</button></div></div>}
        </Section>
      )}

      {activeTab === "Commercial" && commercial && (
        <Section title="Commercial" description="Payment terms, limits and order controls.">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <Field label="Payment Terms"><select value={commercial.payment_term_id ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, payment_term_id: e.target.value || null })} className={inputClass}><option value="">None</option>{paymentTerms.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
            <Field label="Credit Limit"><input type="number" min="0" step="0.01" value={commercial.credit_limit ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, credit_limit: e.target.value || null })} className={inputClass} /></Field>
            <Field label="Minimum Order"><input type="number" min="0" step="0.01" value={commercial.minimum_order_amount ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, minimum_order_amount: e.target.value || null })} className={inputClass} /></Field>
            <Field label="Tax Exemption Number"><input value={commercial.tax_exemption_number ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, tax_exemption_number: e.target.value || null })} className={inputClass} /></Field>
            <Field label="Credit Hold Reason"><input value={commercial.credit_hold_reason ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, credit_hold_reason: e.target.value || null })} className={inputClass} /></Field>
          </div>
          <div className="mt-4 flex gap-5"><Check label="Tax Exempt" checked={commercial.tax_exempt} disabled={!canEdit} onChange={(v) => setCommercial({ ...commercial, tax_exempt: v })} /><Check label="Credit Hold" checked={commercial.credit_hold} disabled={!canEdit} onChange={(v) => setCommercial({ ...commercial, credit_hold: v })} /></div>
          <div className="mt-4 grid gap-4 md:grid-cols-2"><Field label="Discount Notes"><textarea value={commercial.discount_notes ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, discount_notes: e.target.value || null })} className={textareaClass} /></Field><Field label="Order Notes"><textarea value={commercial.order_notes ?? ""} disabled={!canEdit} onChange={(e) => setCommercial({ ...commercial, order_notes: e.target.value || null })} className={textareaClass} /></Field></div>
          {canEdit && <div className="mt-5 flex justify-end"><button onClick={saveCommercial} disabled={isSaving} className={primaryButtonClass}>Save Commercial</button></div>}
        </Section>
      )}

      {activeTab === "Web / Portal" && (
        <Section title="Web / Portal" description="Portal access and users for the customer company.">
          <div className="mb-5 flex items-center justify-between rounded-xl border border-gray-200 p-4 dark:border-gray-800"><div><p className="font-medium text-gray-800 dark:text-white/90">Customer Portal Access</p><p className="mt-1 text-sm text-gray-500">Controls whether this customer can use Modulex Store portal features.</p></div>{canManagePortal ? <button onClick={() => saveCustomer({ portal_enabled: !customer.portal_enabled }, customer.portal_enabled ? "Customer portal disabled" : "Customer portal enabled")} className={customer.portal_enabled ? dangerButtonClass : primaryButtonClass}>{customer.portal_enabled ? "Disable Portal" : "Enable Portal"}</button> : <Badge>{customer.portal_enabled ? "Enabled" : "Disabled"}</Badge>}</div>
          <div className="grid gap-3 lg:grid-cols-2">{portalUsers.map((user) => <Card key={user.id}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-gray-800 dark:text-white/90">{user.full_name || user.login_email}</h3>{user.is_primary && <Badge>Primary</Badge>}</div><p className="mt-1 text-sm text-gray-500">{user.login_email}</p><div className="mt-3 flex gap-2"><Badge>{titleCase(user.portal_role)}</Badge><Badge>{titleCase(user.status)}</Badge></div><p className="mt-3 text-xs text-gray-400">Last login: {dateTime(user.last_login_at)}</p></div>{canManagePortal && <button onClick={() => removePortalUser(user.id)} className={dangerButtonClass}>Remove</button>}</div></Card>)}</div>
          {canManagePortal && <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><h3 className="mb-1 text-sm font-semibold text-gray-800 dark:text-white/90">Add Portal User</h3><p className="mb-4 text-xs text-gray-500">This creates portal metadata only. Auth invitation will be wired through a secure server action after integration testing.</p><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Field label="Full Name"><input value={portalForm.full_name} onChange={(e) => setPortalForm({ ...portalForm, full_name: e.target.value })} className={inputClass} /></Field><Field label="Login Email"><input type="email" value={portalForm.login_email} onChange={(e) => setPortalForm({ ...portalForm, login_email: e.target.value })} className={inputClass} /></Field><Field label="Role"><select value={portalForm.portal_role} onChange={(e) => setPortalForm({ ...portalForm, portal_role: e.target.value as "admin" | "buyer" | "viewer" })} className={inputClass}><option value="admin">Admin</option><option value="buyer">Buyer</option><option value="viewer">Viewer</option></select></Field><Field label="Status"><select value={portalForm.status} onChange={(e) => setPortalForm({ ...portalForm, status: e.target.value as typeof portalForm.status })} className={inputClass}><option value="never_invited">Never Invited</option><option value="invited">Invited</option><option value="active">Active</option><option value="suspended">Suspended</option></select></Field></div><div className="mt-4"><Check label="Primary Portal User" checked={portalForm.is_primary} onChange={(v) => setPortalForm({ ...portalForm, is_primary: v })} /></div><div className="mt-4 flex justify-end"><button onClick={addPortalUser} disabled={isSaving} className={primaryButtonClass}>Add Portal User</button></div></div>}
        </Section>
      )}

      {activeTab === "Notes & Documents" && (
        <Section title="Notes & Documents" description="Internal customer notes and document metadata.">
          <div className="grid gap-6 xl:grid-cols-2"><div><h3 className="mb-3 text-sm font-semibold text-gray-800 dark:text-white/90">Notes</h3><div className="space-y-3">{notes.map((note) => <Card key={note.id}><div className="flex items-start justify-between gap-3"><div>{note.is_pinned && <Badge>Pinned</Badge>}<p className="mt-2 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300">{note.note}</p><p className="mt-2 text-xs text-gray-400">{note.category || "General"} • {dateTime(note.created_at)}</p></div>{canEdit && <button onClick={() => removeNote(note.id)} className={dangerButtonClass}>Remove</button>}</div></Card>)}</div>{canEdit && <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]"><Field label="New Note"><textarea value={noteForm.note} onChange={(e) => setNoteForm({ ...noteForm, note: e.target.value })} className={textareaClass} /></Field><div className="mt-3 grid gap-3 md:grid-cols-2"><Field label="Category"><input value={noteForm.category} onChange={(e) => setNoteForm({ ...noteForm, category: e.target.value })} className={inputClass} /></Field><div className="flex items-end pb-2"><Check label="Pin note" checked={noteForm.is_pinned} onChange={(v) => setNoteForm({ ...noteForm, is_pinned: v })} /></div></div><div className="mt-3 flex justify-end"><button onClick={addNote} className={primaryButtonClass}>Add Note</button></div></div>}</div><div><h3 className="mb-3 text-sm font-semibold text-gray-800 dark:text-white/90">Documents</h3><div className="space-y-3">{documents.length === 0 ? <div className="rounded-xl border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500 dark:border-gray-700">No documents uploaded yet.</div> : documents.map((document) => <Card key={document.id}><p className="font-medium text-gray-800 dark:text-white/90">{document.file_name}</p><p className="mt-1 text-xs text-gray-500">{document.document_type || "Document"} • {dateTime(document.created_at)}</p><p className="mt-2 break-all text-xs text-gray-400">{document.storage_path}</p></Card>)}</div><div className="mt-4 rounded-xl border border-brand-100 bg-brand-25 p-4 text-xs leading-5 text-gray-600 dark:border-brand-500/20 dark:bg-brand-500/[0.06] dark:text-gray-300">Document metadata is ready. File upload will be connected after the <strong>customer-documents</strong> Supabase Storage bucket and its policies are confirmed during integration testing.</div></div></div>
        </Section>
      )}

      {activeTab === "Activity" && (
        <Section title="Activity" description="Append-only customer activity timeline.">
...
function Section({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 sm:p-6"><div className="mb-5"><h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">{title}</h2><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{description}</p></div>{children}</div>;
}
