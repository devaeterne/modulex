...
export default function CustomerInvoicesList({ customerId }: { customerId?: string }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      ...
      <Summary label="Outstanding" value={money(outstanding, invoices[0]?.currency_code ?? "USD")} />
    </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
          <thead className="bg-gray-50 dark:bg-white/[0.02]"><tr>{["Invoice", ...(selectedCustomer ? [] : ["Customer"]), "Order", "Invoice Date", "Due Date", "Status", "Total", "Balance"].map((label) => <th key={label} className="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{label}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {page.map((invoice) => {
              ...
              return <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                ...
                {!selectedCustomer && <td className="px-5 py-4"><Link href={`/customers/${invoice.customer_id}`} className="text-sm font-medium text-gray-800 hover:text-brand-600 dark:text-white/90">{customer?.name ?? "Unknown"}</Link><p className="text-xs text-gray-400">{customer?.customer_code}</p></td>}
                <td className="px-5 py-4 text-sm text-gray-500">{invoice.order_number_snapshot || "—"}</td>
                <td className="px-5 py-4 text-sm text-gray-500">{date(invoice.invoice_date)}</td>
                <td className="px-5 py-4 text-sm text-gray-500">{date(invoice.due_date)}</td>
            ...
            {page.length === 0 && <tr><td colSpan={selectedCustomer ? 7 : 8} className="px-5 py-12 text-center text-sm text-gray-500">No invoices found.</td></tr>}
          </tbody>
        </table>
      </div>
