...
export default function ProjectDocumentsTab({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "Project documents could not be loaded.");
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  async function downloadArtifact(artifact: ProjectProposalArtifact) {
    ...
    try {
      ...
      setError(downloadError instanceof Error ? downloadError.message : "Project document could not be downloaded.");
    } finally {
        ...
        <TableViewport>
          <Table variant="admin" minWidth="standard">
            ...
            <TableBody variant="admin">
              {loading ? <TableStateRow colSpan={5}>Loading Project documents…</TableStateRow> : null}
