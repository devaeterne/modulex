...
const inputClass = "h-10 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30";
...
export default function CustomerInstallationDetail() {
  ...
  if (isLoading) return <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">Loading installation...</div>;
  if (!item) return <div className="rounded-xl border border-error-200 bg-error-50 p-5 text-error-700 dark:border-error-500/30 dark:bg-error-500/10 dark:text-error-400">{errorMessage || "Installation not found."}</div>;

  const address = item.address_snapshot as Record<string, string | null> | null;

  return <div className="space-y-5">
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]"><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold text-gray-800 dark:text-white/90">{item.installation_number}</h1><p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{dateTime(item.scheduled_start_at)}{item.scheduled_end_at ? ` → ${dateTime(item.scheduled_end_at)}` : ""}</p></div><div className="flex gap-2"><Link href={`/customers/${item.customer_id}/orders/${item.order_id}`} className="h-10 rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05]">Open Order</Link><Link href="/customers/installations" className="h-10 rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05]">All Installations</Link></div></div></div>
