  ...
  async function updateState(status?: CustomerInvoiceStatus, explicitPaid?: number) {
    ...
    setSuccessMessage(
      data === "approval_requested"
        ? "Approval requested. The invoice was not changed yet."
        : "Invoice updated."
    );
  }

  if (isLoading) return <Loading />;
  if (!invoice || !customer) return <ErrorBox>{errorMessage || "Invoice not found."}</ErrorBox>;
    ...
    <div className="space-y-5">
      ...
      {pendingApprovals > 0 && (
        <div className="flex flex-col gap-3 rounded-xl border border-warning-200 bg-warning-50 px-4 py-3 text-sm text-warning-700 dark:border-warning-500/30 dark:bg-warning-500/10 dark:text-warning-400 sm:flex-row sm:items-center sm:justify-between">
          <span>{pendingApprovals} protected invoice change{pendingApprovals === 1 ? " is" : "s are"} waiting for approval.</span>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="grid gap-6 border-b border-gray-200 py-6 md:grid-cols-2 dark:border-gray-800">
          <div>
            <p className="mb-2 text-xs font-medium uppercase text-gray-400">Bill To</p>
            <p className="font-semibold text-gray-900 dark:text-white">{customer.name}</p>
            {billing.map((line, index) => <p key={`${line}-${index}`} className="text-sm text-gray-500">{line}</p>)}
          </div>
          <div className="md:text-right">
            <p className="mb-2 text-xs font-medium uppercase text-gray-400">Reference</p>
        ...
        <div className="overflow-x-auto py-6">
          <table className="min-w-full">
            <thead><tr className="border-b border-gray-200 text-left text-xs uppercase text-gray-400 dark:border-gray-800">{["#", "SKU", "Product", "Qty", "Unit Price", "Discount", "Line Total"].map((label) => <th key={label} className={`pb-3 pr-4 ${["Qty", "Unit Price", "Discount", "Line Total"].includes(label) ? "text-right" : ""}`}>{label}</th>)}</tr></thead>
            <tbody>
              {items.length ? items.map((item) => (
                <tr key={item.id} className="border-b border-gray-100 text-sm dark:border-gray-800">
                  <td className="py-3 pr-4 text-gray-400">{item.line_no}</td>
                  <td className="py-3 pr-4 font-medium text-gray-800 dark:text-gray-200">{item.sku_snapshot}</td>
                  <td className="py-3 pr-4 text-gray-700 dark:text-gray-300">{item.product_name_snapshot}</td>
                  <td className="py-3 pr-4 text-right">{Number(item.quantity)}</td>
                  <td className="py-3 pr-4 text-right">{formatMoney(item.unit_price)}</td>
                  <td className="py-3 pr-4 text-right">{Number(item.discount_percent).toFixed(2)}%</td>
                  <td className="py-3 text-right font-medium">{formatMoney(item.line_total)}</td>
                </tr>
              )) : <tr><td colSpan={7} className="py-10 text-center text-sm text-gray-500 dark:text-gray-400">No invoice items found.</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        {settings.invoice_footer_note && <div className="mt-6 whitespace-pre-wrap border-t border-gray-200 pt-4 text-xs text-gray-500 dark:border-gray-800">{settings.invoice_footer_note}</div>}
      </section>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Issue invoices, record payment progress, mark paid or void. Approval rules are enforced by the database workflow.</p>
            </div>
            <div className="flex flex-wrap items-end gap-2">
              {invoice.status === "draft" && <button disabled={isSaving} onClick={() => void updateState("issued")} className="h-10 rounded-lg bg-brand-500 px-4 text-sm font-medium text-white disabled:opacity-60">Issue Invoice</button>}
              {!['draft', 'void'].includes(invoice.status) && (
                <>
                  <label><span className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Paid amount</span><input type="number" min="0" max={Number(invoice.total_amount)} step="0.01" value={paidAmount} onChange={(event) => setPaidAmount(event.target.value)} className="h-10 w-40 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></label>
                  <button disabled={isSaving} onClick={() => void updateState()} className="h-10 rounded-lg border border-gray-300 px-4 text-sm font-medium text-gray-700 disabled:opacity-60 dark:border-gray-700 dark:text-gray-300">Save Payment</button>
                  {invoice.status !== "paid" && <button disabled={isSaving} onClick={() => void updateState("paid", Number(invoice.total_amount))} className="h-10 rounded-lg bg-success-600 px-4 text-sm font-medium text-white disabled:opacity-60">Mark Paid</button>}
                </>
              )}
              {invoice.status !== "void" && <button disabled={isSaving} onClick={() => void updateState("void")} className="h-10 rounded-lg border border-error-300 px-4 text-sm font-medium text-error-600 disabled:opacity-60 dark:border-error-500/40 dark:text-error-400">Void</button>}
            </div>
          </div>
        </section>
      )}
