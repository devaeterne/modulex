...
const statuses: Array<"all" | CustomerInstallationStatus> = [
  "all",
  "scheduled",
  "confirmed",
  "in_progress",
...
export default function CustomerInstallationsList({
  customerId,
}: {
  customerId?: string;
}) {
  ...
  const [status, setStatus] =
    useState<"all" | CustomerInstallationStatus>("all");
  ...
  useEffect(() => {
    async function load() {
      ...
      if (
        ...
        setErrorMessage(
          "You do not have access to installation appointments."
        );
        setIsLoading(false);
        return;
      }
      ...
      const customerIds = [
        ...new Set(
          installations.map((installation) => installation.customer_id)
        ),
      ];

      const orderIds = [
        ...new Set(
          installations.map((installation) => installation.order_id)
        ),
      ];
      ...
      const profileIds = [
        ...new Set(
          installations
            .map((installation) => installation.assigned_to)
            .filter(Boolean) as string[]
        ),
      ];

      const [customersResult, ordersResult, profilesResult] =
        await Promise.all([
          customerIds.length
            ? supabase
              .from("customers")
      ...
      setRows(
        installations.map((item) => ({
          ...
          order_number:
            orderMap.get(item.order_id) ?? null,

          assignee_name: item.assigned_to
            ? profileMap.get(item.assigned_to) ?? null
            : null,
        }))
      );
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="grid gap-3 md:grid-cols-[1fr_220px]">
          <input
            ...
            placeholder="Search appointment, customer, order or team"
            className="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-800 outline-none transition placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
          />
          ...
          <select
            ...
            className="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-800 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-gray-900 dark:text-white/90"
          >
            {statuses.map((item) => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.map((row) => (
                ...
                >
                  <td className="whitespace-nowrap px-4 py-4">
                    <Link
                      href={`/customers/${row.customer_id}/installations/${row.id}`}
                  ...
                  <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    {row.scheduled_end_at && (
                      <div className="mt-1 text-xs text-gray-400 dark:text-gray-500">
                        to {dateTime(row.scheduled_end_at)}
                      </div>
                    )}
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  ...
                  >
                    No installation appointments found.
                  </td>
                </tr>
              )}
            </tbody>
