...
export default function EditCustomerOrder() {
  ...
  useEffect(() => {
    ...
    async function loadPrices() {
      ...
      try {
        ...
        if (!active) return;
        setPrices(data);
        const canonicalPrices = new Map(data.map((price) => [price.product_id, Number(price.amount)]));
        setItems((current) => current.map((item) => {
          const canonicalPrice = canonicalPrices.get(item.product_id);
          return canonicalPrice === undefined ? item : { ...item, unit_price: String(canonicalPrice) };
        }));
      } catch (error) {
        if (!active) return;
  ...
  const preview = useMemo(() => {
    ...
    for (const item of items) {
      ...
      const price = Math.max(0, priceMap.get(item.product_id) ?? 0);
  ...
  }, [items, priceMap, orderDiscount, taxRate, appliedCommission]);
  ...
  async function saveRevision() {
    ...
    for (const item of items) {
      ...
      if (!priceMap.has(item.product_id)) return setErrorMessage(`No current Price Group price exists for ${productMap.get(item.product_id)?.sku ?? "selected product"}.`);
    ...
    try {
      const revision = await updateCustomerOrder({
        ...
        items: items.map((item) => ({
          ...
          unitPrice: String(priceMap.get(item.product_id) ?? 0),
