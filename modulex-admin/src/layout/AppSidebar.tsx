...
type SubItem = { name: string; path: string; permission: Permission; new?: boolean; exact?: boolean };
type NavItem = { name: string; icon: React.ReactNode; path?: string; permission?: Permission; exact?: boolean; subItems?: SubItem[] };

const navItems: NavItem[] = [
  { icon: <GridIcon />, name: "Dashboard", path: "/", permission: "dashboard.view", exact: true },
  { icon: <BoxCubeIcon />, name: "Products", subItems: [
  ...
  { icon: <DollarLineIcon />, name: "Pricing", subItems: [
  ...
  { icon: <UserCircleIcon />, name: "Customers", subItems: [
  ...
  { icon: <TableIcon />, name: "Inventory", subItems: [
  ...
  { icon: <ListIcon />, name: "Warehouse", subItems: [
  ...
  { icon: <PageIcon />, name: "QR Operations", subItems: [
...
const managementItems: NavItem[] = [
  { icon: <UserCircleIcon />, name: "Personnel", subItems: [
  ...
  { icon: <DollarLineIcon />, name: "Finance", subItems: [
  ...
  { icon: <PieChartIcon />, name: "Reports", subItems: [
  ...
  { icon: <PageIcon />, name: "System", subItems: [
...
function filterItems(items: NavItem[], role: UserRole): NavItem[] {
  return items.flatMap((item) => {
    if (item.path) return item.permission && hasPermission(role, item.permission) ? [item] : [];
...
export default function AppSidebar({ role }: { role: UserRole }) {
  ...
  function toggleSubmenu(index: number, type: "main" | "management") { setOpenSubmenu((current) => current?.type === type && current.index === index ? null : { type, index }); }
  useEffect(() => {
    const groups = [{ type: "main" as const, items: visibleNavItems }, { type: "management" as const, items: visibleManagementItems }];
    for (const group of groups) for (let index = 0; index < group.items.length; index += 1) {
      const item = group.items[index];
      if (item.subItems?.some((subItem) => isActive(subItem.path, subItem.exact))) { setOpenSubmenu({ type: group.type, index }); return; }
    }
    setOpenSubmenu(null);
  }, [pathname, isActive, visibleNavItems, visibleManagementItems]);
  const renderItems = (items: NavItem[], type: "main" | "management") => (
    <ul className="flex flex-col gap-4">{items.map((nav, index) => { const isOpen = openSubmenu?.type === type && openSubmenu.index === index; return <li key={nav.name}>{nav.subItems ? <button onClick={() => toggleSubmenu(index, type)} className={`menu-item group ${isOpen ? "menu-item-active" : "menu-item-inactive"} cursor-pointer ${!isExpanded && !isHovered ? "lg:justify-center" : "lg:justify-start"}`}><span className={isOpen ? "menu-item-icon-active" : "menu-item-icon-inactive"}>{nav.icon}</span>{(isExpanded || isHovered || isMobileOpen) && <><span className="menu-item-text">{nav.name}</span><ChevronDownIcon className={`ml-auto h-5 w-5 transition-transform duration-200 ${isOpen ? "rotate-180 text-brand-500" : ""}`} /></>}</button> : nav.path ? <Link href={nav.path} className={`menu-item group ${isActive(nav.path, nav.exact) ? "menu-item-active" : "menu-item-inactive"}`}><span className={isActive(nav.path, nav.exact) ? "menu-item-icon-active" : "menu-item-icon-inactive"}>{nav.icon}</span>{(isExpanded || isHovered || isMobileOpen) && <span className="menu-item-text">{nav.name}</span>}</Link> : null}{nav.subItems && isOpen && (isExpanded || isHovered || isMobileOpen) && <ul className="ml-9 mt-2 space-y-1">{nav.subItems.map((subItem) => <li key={subItem.path}><Link href={subItem.path} className={`menu-dropdown-item ${isActive(subItem.path, subItem.exact) ? "menu-dropdown-item-active" : "menu-dropdown-item-inactive"}`}>{subItem.name}{subItem.new && <span className="menu-dropdown-badge ml-auto">new</span>}</Link></li>)}</ul>}</li>; })}</ul>
  );
  return <aside className={`fixed left-0 top-0 z-50 mt-16 flex h-screen flex-col border-r border-gray-200 bg-white px-5 text-gray-900 transition-all duration-300 ease-in-out dark:border-gray-800 dark:bg-gray-900 lg:mt-0 ${isExpanded || isMobileOpen || isHovered ? "w-[290px]" : "w-[90px]"} ${isMobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`} onMouseEnter={() => !isExpanded && setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}><div className={`flex py-8 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}><Link href="/">{isExpanded || isHovered || isMobileOpen ? <><Image className="dark:hidden" src="/images/logo/logo.svg" alt="Modulex" width={150} height={40} priority /><Image className="hidden dark:block" src="/images/logo/logo-dark.svg" alt="Modulex" width={150} height={40} priority /></> : <Image src="/images/logo/logo-icon.svg" alt="Modulex" width={32} height={32} priority />}</Link></div><div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear"><nav className="mb-6"><div className="flex flex-col gap-4">{visibleNavItems.length > 0 && <div><h2 className={`mb-4 flex text-xs uppercase leading-[20px] text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>{isExpanded || isHovered || isMobileOpen ? "Operations" : <HorizontaLDots />}</h2>{renderItems(visibleNavItems, "main")}</div>}{visibleManagementItems.length > 0 && <div><h2 className={`mb-4 flex text-xs uppercase leading-[20px] text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>{isExpanded || isHovered || isMobileOpen ? "Management" : <HorizontaLDots />}</h2>{renderItems(visibleManagementItems, "management")}</div>}</div></nav>{isExpanded || isHovered || isMobileOpen ? <SidebarWidget /> : null}</div></aside>;
}
