...
A2.4 production acceptance: **PASS / CLOSED**
...
- Production deployment `dpl_Ghtw7cnxmAUnkF2s4794cD2L7YAF` is READY at Git SHA `2d08d28a0efbcb6dd9df390f351cbdfb38513fca` (PR #176 merge).
- Authenticated browser smoke passed for `/low-stock`, `/reports/inventory`, and `/reports/movements`: all routes rendered with the expected title, Super Admin auth/RBAC was retained, visible search/filter controls and deterministic pagination behaved correctly, production RPC requests returned HTTP 200, and no console/runtime errors were observed. Inventory search and tab/filter changes produced consistent counts/results; request sequencing is guarded by the deployed request-id checks.
- CSV smoke passed: Low Stock correctly kept Export Alerts disabled because the filtered alert dataset is empty; Inventory Product export returned the three `CSB36` filtered rows; Inventory Location export returned both production locations; Movement export returned the four filtered production events. Exports were generated by paged RPC loops and were not limited to the visible page or a 1,000-row client query.
