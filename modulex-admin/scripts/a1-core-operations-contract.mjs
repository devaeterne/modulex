...
assert.match(orderDetail, /variant="admin">By<\/TableCell>/, "Order status timeline must expose who made the change");
assert.match(orderDetail, /variant="admin">Note<\/TableCell>/, "Order status timeline must expose status notes");
assert.match(orderDetail, /\.from\("profiles"\)[\s\S]{0,180}\.in\("id", actorIds\)/, "Order status timeline must resolve changed_by actors through a bounded profile lookup");
assert.match(orderDetail, /entry\.changed_by/, "Order status timeline must derive actor identity from changed_by");
