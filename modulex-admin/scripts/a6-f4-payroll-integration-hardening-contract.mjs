...
const financeCorePath = path.join(here, "../src/lib/finance/core.ts");
...
const financeCore = readFileSync(financeCorePath, "utf8");
...
assert.match(
  sql,
  /Finance payment would exceed the Variable Pay amount/i,
  "Direct Variable Pay settlement must prevent overpayment",
);
assert.match(
  sql,
  /Finance payment would exceed the Advance amount/i,
  "Direct Advance disbursement must prevent duplicate overpayment",
);
assert.match(
  sql,
  /status\s*=\s*'paid'/i,
  "Direct Variable Pay settlement must mark fully paid HR sources paid",
);
assert.match(
  sql,
  /status\s*=\s*'approved'/i,
  "Direct Variable Pay reversal must restore source eligibility",
);
assert.match(
  sql,
  /company base currency/i,
  "Payroll preparation must fail closed on unsupported source currency",
);
assert.match(
  sql,
...
assert.match(
  sql,
  /language\s+sql\s+security\s+definer[\s\S]*set\s+search_path\s+to\s+''/i,
  "Public Finance bridge RPCs must use SECURITY DEFINER with a pinned empty search_path",
);
assert.match(
  sql,
  /revoke\s+all\s+on\s+function\s+public\.save_employee_payment_draft/i,
  "Atomic employee payment draft RPC must revoke broad execute access",
);
assert.match(
  sql,
...
assert.ok(
  financeCore.includes("saveEmployeePaymentDraft"),
  "Finance client must expose the atomic employee payment draft RPC",
);
assert.ok(
  financeCore.includes("getFinancePayrollObligations"),
  "Finance client must expose the read-only payroll obligation projection",
);
assert.ok(
  financeManager.includes("saveEmployeePaymentDraft"),
  "Finance Employee Payment UI must use the atomic draft RPC",
);
assert.ok(
  !financeManager.includes("Employee/Payroll link failed"),
  "Finance UI must no longer expose the known two-step orphan-draft failure mode",
);
assert.ok(
  payrollManager.includes("Pay Remaining"),
  "Payroll UI must offer a Pay Remaining handoff to Finance",
);
assert.ok(
  payrollManager.includes("payrollItemId"),
  "Pay Remaining handoff must preserve Payroll Item context",
);
