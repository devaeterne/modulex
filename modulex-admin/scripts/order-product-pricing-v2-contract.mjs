...
assert(/createOrder\(validItems,\s*header,\s*"draft"\)/.test(createOrder) && createOrder.includes("<CountertopConfigurator"), "New Order Countertop action must save an exact-validated Draft shell before opening the canonical configurator");
...
assert(editOrder.includes("resolveOrderLineUnitPriceValue"), "Edit Order must resolve line money through one pricing-model-aware exact-value helper");
assert(/model\s*===\s*"price_group"[\s\S]{0,180}priceValueMap\.get\(item\.product_id\)/.test(editOrder), "Price Group edit lines must preserve canonical price strings for mutation truth");
...
assert(/unitPrice:\s*unitPrice\.value/.test(editOrder) && /parseOrderMoney\(unitPriceValue\)/.test(editOrder), "Edit Order submit payload must use exact validated canonical money");
assert(/price\s*=\s*Math\.max\(0,\s*Number\(resolveOrderLineUnitPriceValue/.test(editOrder), "Edit Order preview totals may convert exact canonical money only for UI preview");
