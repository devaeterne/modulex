...
assert.match(configSource, /\/api\/admin\/google-calendar\/oauth\/callback/);
assert.match(configSource, /\/api\/integrations\/google\/calendar\/callback/);
...
assert.match(repositorySource, /provider_account_id:\s*null/);
assert.match(repositorySource, /provider_account_email:\s*null/);
assert.match(repositorySource, /granted_scopes:\s*\[\]/);
...
assert.match(oauthSource, /url\.searchParams\.set\("prompt", "consent select_account"\)/);
...
assert.doesNotMatch(oauthStartRoute, /loginHint:\s*auth\.actor\.user\.email/);
...
assert.match(statusRoute, /provider_account_email:\s*credential\?\.status === "connected"/);
