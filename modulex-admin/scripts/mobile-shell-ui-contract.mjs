...
expect(workflow.includes('- "modulex-admin/**"'), "Consolidated Admin UI workflow must watch Admin feature changes");
expect(workflow.includes("name: Mobile shell UI contract") && workflow.includes("run: node scripts/mobile-shell-ui-contract.mjs"), "Admin UI Foundation must run the Mobile shell UI contract after workflow consolidation");
