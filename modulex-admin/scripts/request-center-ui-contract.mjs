...
expect(center.includes('import { formatDateTime } from "@/lib/dates/usDate"'), "Request dates must use the shared deterministic US date formatter");
expect(center.includes("return formatDateTime(value)"), "Request date rendering must delegate to the shared deterministic US date formatter");
