...
assert(
  /sales:\s*\[[\s\S]*?"project_payments\.view"/.test(permissions),
  "Sales must receive project_payments.view"
);
assert(
  !/sales:\s*\[[\s\S]*?"project_payments\.manage"/.test(permissions),
  "Sales must not receive project_payments.manage"
);
assert(
  /finance:\s*\[[\s\S]*?"project_payments\.manage"/.test(permissions),
  "Finance must receive project_payments.manage"
);

assert(
  paymentDomain.includes('.rpc("get_customer_project_payment_ledger"'),
  "Finance/Admin must use authoritative Project payment ledger RPC"
);
assert(
  paymentDomain.includes('.rpc("create_customer_project_payment_requirement"'),
  "Project payment domain must create requirements through RPC"
);
assert(
  paymentDomain.includes('.rpc("record_customer_project_payment"'),
  "Project payment domain must record actual payments through RPC"
);
assert(
  paymentDomain.includes('.rpc("allocate_customer_project_payment"'),
  "Project payment domain must allocate payments through RPC"
);
assert(
  paymentDomain.includes('.rpc("reverse_customer_project_payment"'),
  "Project payment domain must reverse payments through RPC"
);
assert(
  paymentStatusDomain.includes('.rpc("get_customer_project_payment_status"'),
  "Sales must use sanitized Project payment status RPC"
);
...
for (const forbidden of [
  ...
  assert(
    !new RegExp(`\\b${forbidden}\\b`, "i").test(paymentStatusDomain),
    `Sales payment status adapter must not model restricted field ${forbidden}`
  );
}
...
assert(
  invoiceDetail.includes("Legacy payment tracking") || invoiceDetail.includes("ledgerManaged"),
  "Invoice detail must preserve an explicit legacy compatibility path"
);
...
for (const functionName of [
  ...
  assert(
    migration.includes(functionName) || hardening.includes(functionName),
    `PB-3A SQL must define ${functionName}`
  );
}
assert(migration.includes("ledger_managed"), "PB-3A migration must add ledger_managed invoice compatibility state");
assert(
  migration.includes("customer_invoices") && migration.includes("paid_amount"),
  "PB-3A migration must reconcile ledger-managed invoices to paid_amount compatibility projection"
);
assert(
  !migration.toLowerCase().includes("insert into public.customer_project_payment_transactions select"),
