...
assert.match(sync, /categoryKey\s*===\s*null|!categoryKey/);
...
assert.match(approve, /availability_status/);
assert.match(approve, /VendorUnavailableError/);
assert.match(approve, /isVendorApprovalEligible/);
...
assert.match(eligibleRoute, /availability_status/);
assert.match(eligibleRoute, /AVAILABLE/);
assert.match(eligibleRoute, /vendor_catalog_category_mappings/);
...
assert.match(bulkApproveRoute, /concurrency/i);
assert.match(bulkApproveRoute, /2/);
assert.match(bulkApproveRoute, /approveVendorCatalogItem/);
assert.match(bulkApproveRoute, /SKIPPED/);

assert.match(page, /availability_status/);
assert.match(page, /Stock \/ Availability|Availability/);
assert.match(page, /Select all .*eligible filtered products/i);
assert.match(page, /Approve Selected/);
assert.match(page, /Approved .* of/i);
...
assert.match(migration, /OUT_OF_STOCK/);
