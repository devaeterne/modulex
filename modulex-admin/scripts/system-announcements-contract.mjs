...
assert.match(model, /SYSTEM_ANNOUNCEMENTS_REFRESH_EVENT/, "Announcement model must expose the shared refresh event name");
...
assert.match(publisher, /dispatchEvent\(new Event\(SYSTEM_ANNOUNCEMENTS_REFRESH_EVENT\)\)/, "Publishing must request an immediate notification refresh");
assert.match(notifications, /addEventListener\(SYSTEM_ANNOUNCEMENTS_REFRESH_EVENT/, "Notification dropdown must listen for immediate announcement refreshes");
assert.match(notifications, /removeEventListener\(SYSTEM_ANNOUNCEMENTS_REFRESH_EVENT/, "Notification dropdown must clean up the announcement refresh listener");
