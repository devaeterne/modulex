...
assert(runtimeMigration.includes("create or replace function private.audit_countertop_reference_change") && runtimeMigration.includes("to_jsonb(old)") && runtimeMigration.includes("to_jsonb(new)") && runtimeMigration.includes("::public.audit_action"), "runtime audit must resolve generic trigger rows through JSON and cast the audit enum");
