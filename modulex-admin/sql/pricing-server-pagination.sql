-- Server-side read models for Product Prices and Cost & Margin.
-- Keeps existing write RPCs unchanged and preserves RLS with SECURITY INVOKER.
...
with params as (
  select
    ...
    case when lower(coalesce(p_sort_by, 'sku')) in ('sku', 'name', 'brand', 'category', 'stock', 'status')
...
create or replace function public.get_product_prices_for_products(
  p_product_ids uuid[],
  p_currency_code text default 'USD'
)
returns table(product_id uuid, price_group_id uuid, amount numeric)
language sql
stable
security invoker
set search_path = public
as $$
  select pp.product_id, pp.price_group_id, pp.amount
  from public.product_prices pp
  join public.price_groups pg on pg.id = pp.price_group_id and pg.is_active = true
  ...
  select b.*,
    case
      ...
      when exists(
        ...
        where cp.product_id = b.product_id and cp.amount > 0
          and (((cp.amount - b.cost_amount) / cp.amount) * 100) < 0
      ) then 'loss'
      when exists(
        select 1 from current_prices cp
        where cp.product_id = b.product_id and cp.amount > 0
          and (((cp.amount - b.cost_amount) / cp.amount) * 100) < (b.effective_min_margin - b.warning_buffer)
      ) then 'critical'
      when exists(
        select 1 from current_prices cp
        where cp.product_id = b.product_id and cp.amount > 0
          and (((cp.amount - b.cost_amount) / cp.amount) * 100) < b.effective_min_margin
      ) then 'warning'
      when exists(
        select 1 from current_prices cp
        where cp.product_id = b.product_id and cp.amount > 0
    ...
    (
      select min(((cp.amount - b.cost_amount) / cp.amount) * 100)
      from current_prices cp
      where cp.product_id = b.product_id
        and cp.amount > 0
        and b.cost_amount is not null
    ) as worst_margin
  from base0 b
),
filtered as (
...
create or replace function public.get_product_costs_for_products(
  p_product_ids uuid[],
  p_currency_code text default 'USD'
)
...
as $$
  ...
  where pc.product_id = any(coalesce(p_product_ids, '{}'::uuid[]))
    ...
    and pc.valid_to is null
    and pc.currency_code = upper(coalesce(nullif(btrim(p_currency_code), ''), 'USD'));
$$;

revoke all on function public.get_product_prices_page(text, integer, integer, text, uuid, uuid, text, text, text, text) from public;
revoke all on function public.get_product_prices_page(text, integer, integer, text, uuid, uuid, text, text, text, text) from anon;
grant execute on function public.get_product_prices_page(text, integer, integer, text, uuid, uuid, text, text, text, text) to authenticated;

revoke all on function public.get_product_prices_for_products(uuid[], text) from public;
revoke all on function public.get_product_prices_for_products(uuid[], text) from anon;
grant execute on function public.get_product_prices_for_products(uuid[], text) to authenticated;

revoke all on function public.get_cost_margin_page(text, integer, integer, text, uuid, uuid, text, text, text, text, text) from public;
revoke all on function public.get_cost_margin_page(text, integer, integer, text, uuid, uuid, text, text, text, text, text) from anon;
grant execute on function public.get_cost_margin_page(text, integer, integer, text, uuid, uuid, text, text, text, text, text) to authenticated;

revoke all on function public.get_product_costs_for_products(uuid[], text) from public;
revoke all on function public.get_product_costs_for_products(uuid[], text) from anon;
grant execute on function public.get_product_costs_for_products(uuid[], text) to authenticated;
