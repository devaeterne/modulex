...
-- ============================================================
-- MODULEX CUSTOMER ORDER PAYMENT METHODS
-- Safe additive migration for existing customer_orders.
-- ============================================================
...
create table if not exists public.payment_methods (
  ...
  constraint payment_methods_key_not_empty check (length(trim(system_key)) > 0),
  constraint payment_methods_name_not_empty check (length(trim(name)) > 0),
  constraint payment_methods_commission_range check (commission_percent >= 0 and commission_percent <= 100)
);

insert into public.payment_methods (system_key, name, commission_percent, sort_order)
values
  ('cash', 'Cash', 0, 10),
  ('card', 'Card', 0, 20),
  ('bank_transfer', 'Bank Transfer', 0, 30)
on conflict (system_key) do nothing;

create or replace function public.set_payment_method_updated_metadata()
returns trigger
language plpgsql
security invoker
...
for each row execute function public.set_payment_method_updated_metadata();
...
alter table public.customer_orders
  ...
  add column if not exists payment_commission_percent numeric(7,3) not null default 0,
  add column if not exists payment_commission_amount numeric(18,4) not null default 0,
  add column if not exists grand_total numeric(18,4) not null default 0;
...
alter table public.customer_orders
  drop constraint if exists customer_orders_payment_commission_range;

alter table public.customer_orders
  add constraint customer_orders_payment_commission_range
  check (
    payment_commission_percent >= 0
    and payment_commission_percent <= 100
    and payment_commission_amount >= 0
    and grand_total >= 0
  );

create index if not exists customer_orders_payment_method_idx
  on public.customer_orders(payment_method_id);

-- ============================================================
-- RLS / GRANTS FOR PAYMENT METHODS
-- Read: admin/sales. Manage: admin/super_admin.
-- ============================================================
...
-- ============================================================
-- REPLACE CREATE ORDER RPC WITH PAYMENT METHOD SUPPORT
-- Existing order pricing behaviour is preserved.
-- Commission is calculated AFTER discount + tax.
-- ============================================================
...
declare
  ...
  v_payment_commission_percent numeric(7,3) := 0;
...
begin
  ...
  if p_payment_method_id is null then
    ...
    into v_payment_method_id, v_payment_method_name, v_payment_commission_percent
  ...
  insert into public.customer_orders (
    ...
    v_payment_method_id, v_payment_method_name, v_payment_commission_percent,
  ...
  v_payment_commission_amount := round(v_total * (v_payment_commission_percent / 100), 4);
